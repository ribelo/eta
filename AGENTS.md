...
- [.reference/incremental](/home/ribelo/projects/ribelo/ocaml/Eta/.reference/incremental) - Jane Street Incremental reference for fine-grained reactive graph semantics: generative graph instances, observed-only recomputation, explicit stabilization, topological/height-ordered recompute heaps, cutoffs, dynamic dependencies through `bind`, invalidation, cycle detection, observer update timing, and graph liveness. Use it as prior art for `eta_signal` design and tests, while keeping Eta's package boundary and effect contract instead of depending on Jane Street libraries or copying the full Incremental surface.
