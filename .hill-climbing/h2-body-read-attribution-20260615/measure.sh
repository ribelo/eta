...
def parse_client(path):
    ...
    with path.open(newline="") as f:
        ...
        for row in reader:
            ...
            streams.add(int(row["stream_id"]))
    if len(streams) != requests:
        raise SystemExit(f"{path} expected {requests} streams, saw {len(streams)}")
    return streams
...
segment_names = [
    ...
    "first_read_call_to_first_arm",
    "data_to_first_arm",
    "first_arm_to_data",
    "first_arm_to_chunk_callback",
    "data_to_chunk_callback",
    "reader_delivery_after_ready",
    "chunk_callback_to_resolved",
    "chunk_callback_to_return",
    "chunk_return_to_eof_call",
    "second_read_call_to_second_arm",
    "eof_call_to_eof_return",
    "eof_armed_to_eof_callback",
...
for repeat in range(1, repeats + 1):
    client_streams = parse_client(result_dir / f"client-{repeat}.tsv")
    trace = parse_trace(result_dir / f"server-trace-{repeat}.log")
    values = {name: [] for name in segment_names}
    for stream_id in sorted(client_streams):
        s = trace.get(stream_id)
        if s is None:
            raise SystemExit(f"repeat {repeat} stream {stream_id} missing trace")
        required_scalar = ["handler_started", "body_available", "request_body_read"]
        if any(name not in s for name in required_scalar):
            raise SystemExit(f"repeat {repeat} stream {stream_id} missing handler fields: {s}")
        if not s["data_frames"] or len(s["read_calls"]) < 2 or len(s["read_armed"]) < 2 or not s["chunks"] or not s["eofs"]:
        ...
        if not chunk_returns or not eof_returns:
            raise SystemExit(f"repeat {repeat} stream {stream_id} missing read returns: {s}")
        ...
        row = {
            ...
            "first_read_call_to_first_arm": read_armed[0] - read_calls[0],
            "data_to_first_arm": read_armed[0] - data["data_arrived_us"],
            "first_arm_to_data": data["data_arrived_us"] - read_armed[0],
            "first_arm_to_chunk_callback": chunk["callback_us"] - read_armed[0],
            "data_to_chunk_callback": chunk["callback_us"] - data["data_arrived_us"],
            "reader_delivery_after_ready": chunk["callback_us"] - ready_for_delivery_us,
            "chunk_callback_to_resolved": chunk["resolved_us"] - chunk["callback_us"],
            "chunk_callback_to_return": chunk_return["read_return_us"] - chunk["callback_us"],
            "chunk_return_to_eof_call": read_calls[1] - chunk_return["read_return_us"],
            "second_read_call_to_second_arm": read_armed[1] - read_calls[1],
            "eof_call_to_eof_return": eof_return["read_return_us"] - read_calls[1],
            "eof_armed_to_eof_callback": eof["eof_us"] - read_armed[1],
    ...
    tail_n = max(1, math.ceil(requests * 0.01))
    tail_rows = sorted(
        [row for row in all_rows if row["repeat"] == repeat],
        key=lambda row: row["handler_to_available"],
        reverse=True,
    )[:tail_n]
    dominant = {
        "handler_to_read_call": 0,
        "read_command": 0,
        "wait_for_data": 0,
        "reader_delivery": 0,
        "chunk_return": 0,
        "second_read_command": 0,
        "eof_callback": 0,
        "eof_return": 0,
        "post_eof": 0,
    }
    for row in tail_rows:
        parts = {
            "handler_to_read_call": max(0, row["handler_to_first_read_call"]),
            "read_command": max(0, row["first_read_call_to_first_arm"]),
            "wait_for_data": max(0, row["first_arm_to_data"]),
            "reader_delivery": max(0, row["reader_delivery_after_ready"]),
            "chunk_return": max(0, row["chunk_callback_to_return"]),
            "second_read_command": max(0, row["second_read_call_to_second_arm"]),
            "eof_callback": max(0, row["eof_armed_to_eof_callback"]),
            "eof_return": max(0, row["eof_callback_to_return"]),
            "post_eof": max(0, row["eof_return_to_body_available"]),
        }
        dominant[max(parts, key=parts.get)] += 1
    for name, count in dominant.items():
        summary[f"tail1pct_dominant_{name}_fraction"] = count / tail_n
    summary["tail1pct_handler_to_available_median_us"] = statistics.median(
        row["handler_to_available"] for row in tail_rows
    )
    summaries.append(summary)

summary_path = result_dir / "body-read-summary.tsv"
...
samples_path = result_dir / "body-read-samples.tsv"
...
print("repeat\thandler_available_p99_us\tread_call_p99_us\tread_arm_p99_us\tarm_data_p99_us\tready_delivery_p99_us\tchunk_return_p99_us\tsecond_read_arm_p99_us\teof_callback_p99_us\teof_return_p99_us\tpost_eof_p99_us")
for summary in summaries:
    print(
        f"{summary['repeat']}\t"
        f"{summary['handler_to_available_p99_us']}\t"
        f"{summary['handler_to_first_read_call_p99_us']}\t"
        f"{summary['first_read_call_to_first_arm_p99_us']}\t"
        f"{summary['first_arm_to_data_p99_us']}\t"
        f"{summary['reader_delivery_after_ready_p99_us']}\t"
        f"{summary['chunk_callback_to_return_p99_us']}\t"
        f"{summary['second_read_call_to_second_arm_p99_us']}\t"
        f"{summary['eof_armed_to_eof_callback_p99_us']}\t"
...
top_text = ",".join(
    ...
    f"/call={row['handler_to_first_read_call']}us"
    f"/cmd={row['first_read_call_to_first_arm']}us"
    f"/arm_data={row['first_arm_to_data']}us"
    f"/delivery={row['reader_delivery_after_ready']}us"
    f"/chunk_return={row['chunk_callback_to_return']}us"
    f"/second_cmd={row['second_read_call_to_second_arm']}us"
    f"/eof_cb={row['eof_armed_to_eof_callback']}us"
...
metrics = {
    ...
    "h2_body_handler_to_first_read_call_p99_us": median_metric(summaries, "handler_to_first_read_call_p99_us"),
    "h2_body_first_read_call_to_arm_p99_us": median_metric(summaries, "first_read_call_to_first_arm_p99_us"),
    "h2_body_data_to_first_arm_p99_us": median_metric(summaries, "data_to_first_arm_p99_us"),
    "h2_body_first_arm_to_data_p99_us": median_metric(summaries, "first_arm_to_data_p99_us"),
    "h2_body_first_arm_to_chunk_callback_p99_us": median_metric(summaries, "first_arm_to_chunk_callback_p99_us"),
    "h2_body_data_to_chunk_callback_p99_us": median_metric(summaries, "data_to_chunk_callback_p99_us"),
    "h2_body_reader_delivery_after_ready_p99_us": median_metric(summaries, "reader_delivery_after_ready_p99_us"),
    "h2_body_chunk_callback_to_resolved_p99_us": median_metric(summaries, "chunk_callback_to_resolved_p99_us"),
    ...
    "h2_body_second_read_call_to_arm_p99_us": median_metric(summaries, "second_read_call_to_second_arm_p99_us"),
    "h2_body_eof_call_to_eof_return_p99_us": median_metric(summaries, "eof_call_to_eof_return_p99_us"),
    "h2_body_eof_armed_to_eof_callback_p99_us": median_metric(summaries, "eof_armed_to_eof_callback_p99_us"),
    ...
    "h2_body_tail1pct_dominant_handler_to_read_call_fraction": median_metric(summaries, "tail1pct_dominant_handler_to_read_call_fraction"),
    "h2_body_tail1pct_dominant_read_command_fraction": median_metric(summaries, "tail1pct_dominant_read_command_fraction"),
    "h2_body_tail1pct_dominant_wait_for_data_fraction": median_metric(summaries, "tail1pct_dominant_wait_for_data_fraction"),
    "h2_body_tail1pct_dominant_reader_delivery_fraction": median_metric(summaries, "tail1pct_dominant_reader_delivery_fraction"),
    "h2_body_tail1pct_dominant_chunk_return_fraction": median_metric(summaries, "tail1pct_dominant_chunk_return_fraction"),
    "h2_body_tail1pct_dominant_second_read_command_fraction": median_metric(summaries, "tail1pct_dominant_second_read_command_fraction"),
    "h2_body_tail1pct_dominant_eof_callback_fraction": median_metric(summaries, "tail1pct_dominant_eof_callback_fraction"),
    "h2_body_tail1pct_dominant_eof_return_fraction": median_metric(summaries, "tail1pct_dominant_eof_return_fraction"),
    "h2_body_tail1pct_dominant_post_eof_fraction": median_metric(summaries, "tail1pct_dominant_post_eof_fraction"),
    "h2_body_success": 1.0,
}
for name, value in metrics.items():
    print(f"METRIC {name}={value:.6f}")
print(f"body-read samples: {samples_path}", file=sys.stderr)
print(f"body-read summary: {summary_path}", file=sys.stderr)
PY
