...
REQUESTS="${ETA_H2PLAIN_REQUESTS:-20000}"
REPS="${ETA_H2PLAIN_REPS:-7}"
TIMEOUT="${ETA_H2PLAIN_TIMEOUT:-5s}"
WARMUP_REQUESTS="${ETA_H2PLAIN_WARMUP_REQUESTS:-2000}"
...
# Warm the H2C path and the echo request/response body path before sampling.
run_oha echo_1k POST /echo "$BODY" "$WARMUP_REQUESTS" "$TMP/warmup.json"

for repeat in $(seq 1 "$REPS"); do
  run_oha echo_1k POST /echo "$BODY" "$REQUESTS" "$TMP/echo_1k-$repeat.json"
  record_sample echo_1k "$TMP/echo_1k-$repeat.json"
  run_oha root GET / "" "$REQUESTS" "$TMP/root-$repeat.json"
  record_sample root "$TMP/root-$repeat.json"
  run_oha post_user POST /user "" "$REQUESTS" "$TMP/post_user-$repeat.json"
  record_sample post_user "$TMP/post_user-$repeat.json"
  run_oha static_1k GET /static/1k.bin "" "$REQUESTS" "$TMP/static_1k-$repeat.json"
  record_sample static_1k "$TMP/static_1k-$repeat.json"
done
