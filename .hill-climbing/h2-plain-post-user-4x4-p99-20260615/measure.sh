...
REQUESTS="${ETA_H2_POST_4X4_REQUESTS:-24000}"
REPEATS="${ETA_H2_POST_4X4_REPEATS:-9}"
CONNECTIONS="${ETA_H2_POST_4X4_CONNECTIONS:-4}"
STREAMS="${ETA_H2_POST_4X4_STREAMS:-4}"
TIMEOUT="${ETA_H2_POST_4X4_TIMEOUT:-10s}"
...
run_endpoint_set() {
  ...
  for repeat in $(seq 1 "$REPEATS"); do
    echo "$server repeat $repeat/$REPEATS: H2C 4x4 root/user/post/static/echo" >&2
    local spec
...
metrics = {}
metrics["h2_plain_post_user_4x4_success"] = 1.0 if all(row["ok"] for row in rows) else 0.0
...
for endpoint, _expected_body_bytes in endpoints:
    ...
    for server in servers:
        ...
        prefix = f"h2_plain_{server}_{endpoint}_4x4"
    ...
    metrics[f"h2_plain_{endpoint}_4x4_eta_node_rps_ratio"] = rps_ratio
    metrics[f"h2_plain_{endpoint}_4x4_eta_node_p99_ratio"] = p99_ratio
...
metrics["h2_plain_post_user_4x4_eta_node_p99_ratio"] = metrics[
    "h2_plain_post_user_4x4_eta_node_p99_ratio"
]
metrics["h2_plain_post_user_4x4_eta_node_rps_ratio"] = metrics[
    "h2_plain_post_user_4x4_eta_node_rps_ratio"
]
metrics["h2_plain_post_user_4x4_eta_p99_us"] = metrics[
    "h2_plain_eta_post_user_4x4_p99_us"
]
metrics["h2_plain_post_user_4x4_node_p99_us"] = metrics[
    "h2_plain_node_post_user_4x4_p99_us"
]
metrics["h2_plain_post_user_4x4_eta_rps"] = metrics[
    "h2_plain_eta_post_user_4x4_rps"
]
metrics["h2_plain_post_user_4x4_node_rps"] = metrics[
    "h2_plain_node_post_user_4x4_rps"
]
metrics["h2_plain_nonpost_4x4_eta_node_p99_ratio_geomean"] = geomean(
    [
        row["eta_node_p99_ratio"]
        for row in summary_rows
        if row["endpoint"] != "post_user"
    ]
)
