...
type row = {
  ...
  certs : server_certs;
  identity : identity;
  server_version : tls_version * tls_version;
  expected : expected;
}
...
let cert_root_name () =
  "h_s3_pivot_local_cert_" ^ string_of_int (Unix.getpid ())
  ...
  openssl
    (String.concat " "
       [
         ...
         "-days 30 -sha256";
         "-subj " ^ quote "/CN=Eta H-S3 Pivot Test CA";
...
let prepare_certs () =
  ...
  mkdir_p root;
  create_ca root;
  create_leaf root ~name:"single" ~cn:"api.local.test"
    ~dns:[ "api.local.test" ] ~ips:[];
  create_leaf root ~name:"wildcard" ~cn:"*.wild.local.test"
    ~dns:[ "*.wild.local.test" ] ~ips:[];
  create_leaf root ~name:"multi" ~cn:"api.local.test"
    ~dns:[ "api.local.test"; "multi.local.test" ] ~ips:[];
  create_leaf root ~name:"ip" ~cn:"127.0.0.1" ~dns:[]
    ~ips:[ "127.0.0.1" ];
  create_leaf root ~name:"idna" ~cn:"xn--bcher-kva.local.test"
    ~dns:[ "xn--bcher-kva.local.test" ] ~ips:[];
  create_leaf root ~name:"default" ~cn:"default.local.test"
    ~dns:[ "default.local.test" ] ~ips:[];
  create_leaf root ~name:"sni" ~cn:"sni.local.test"
    ~dns:[ "sni.local.test" ] ~ips:[];
  root_name

let certchain dir name =
  let open Eio.Path in
  X509_eio.private_of_pems ~cert:(dir / (name ^ ".pem"))
...
let server_config dir row =
  Tls.Config.server ~version:row.server_version
    ~certificates:(own_cert dir row.certs) ~ciphers:narrowed_ciphers ()
...
let client_config dir row =
  ...
  | Ip ip ->
      Tls.Config.client ~authenticator ~version:policy_version
        ~ciphers:narrowed_ciphers ~ip:(Ipaddr.of_string_exn ip) ()
...
let classify_error ~policy_expected exn =
  ...
  then "reject_name"
    ...
    contains ~needle:"handshake failure" error
    || contains ~needle:"protocol version" error
    || contains ~needle:"no configured ciphersuite" error
    || policy_expected
  then "reject_policy"
  else "reject_other"
...
let run_row env dir row =
  ...
  Eio.Fiber.fork ~sw (fun () ->
      let result =
        try
          ...
          let tls_flow =
            Tls_eio.server_of_flow (server_config dir row) raw_flow
          in
  ...
  let outcome =
    try connect_client net dir row port
    with exn ->
      Rejected (classify_error ~policy_expected:(row.expected = Reject_policy) exn)
  in
...
let matches = function
  | Accept, Accepted { payload = "ok"; version = "tls12" } -> true
  | Accept, _ -> false
  | Reject_name, Rejected "reject_name" -> true
  | Reject_name, _ -> false
  | Reject_policy, Rejected "reject_policy" -> true
  | Reject_policy, _ -> false
...
let rows =
  [
    ...
    };
    {
      ...
      certs = Single "single";
      identity = Host "api.local.test";
      server_version = (`TLS_1_2, `TLS_1_2);
      expected = Accept;
    };
    {
      name = "tls13_only_rejected_by_policy";
      certs = Single "single";
      identity = Host "api.local.test";
      server_version = (`TLS_1_3, `TLS_1_3);
      expected = Reject_policy;
    };
  ]
...
let () =
  ...
  let failures =
    List.filter_map
      (fun row ->
        ...
        let result = if matches (row.expected, outcome) then "PASS" else "FAIL" in
        (match outcome with
        | Accepted { version; payload } ->
            Printf.printf
              "h_s3_pivot_local_cert name=%s expected=%s observed=%s result=%s identity=%s version=%s payload=%S policy=tls12_ecdhe_aead_only\n\
               %!"
              row.name (expected_label row.expected)
              (observed_of_outcome outcome) result
              (string_of_identity row.identity) version payload
        | Rejected class_ ->
            Printf.printf
              "h_s3_pivot_local_cert name=%s expected=%s observed=%s result=%s identity=%s detail=%S policy=tls12_ecdhe_aead_only\n\
               %!"
              row.name (expected_label row.expected)
              (observed_of_outcome outcome) result
              (string_of_identity row.identity) class_);
  ...
  | failures ->
      Printf.printf
        "h_s3_pivot_local_cert_summary verdict=FAIL failed=%s policy=tls12_ecdhe_aead_only\n\
         %!"
