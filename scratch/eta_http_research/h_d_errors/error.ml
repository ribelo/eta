...
let layer t =
  ...
  | Hpack_decode_overflow _ | Continuation_flood _
  | Stream_admission_rejected _ | Rst_rate_exceeded _ ->
      Http_response

let retryability t =
  match t.kind with
  | Tls_certificate_error _ | Decode_error _ -> Not_retryable
  ...
  | Connect_timeout _ | Pool_acquire_timeout _ | Response_header_timeout _
  | Response_body_idle_timeout _ | Total_request_timeout _
  | Connection_closed _ | Tls_handshake_error _ ->
      Retryable_if_body_replayable

let status t =
  match t.kind with
  | HTTP_status { status; _ } -> Some status
  | _ -> None
...
let error_class t =
  ...
  | Tls_handshake_error { stage = Alpn_negotiation; _ } -> "alpn_negotiation_error"
  | Tls_certificate_error _ -> "tls_certificate_error"
  | Connection_closed _ -> "connection_closed"
  | Pool_shutdown -> "pool_shutdown"
  | Pool_acquire_timeout _ -> "pool_acquire_timeout"
...
let headers t =
  match t.kind with
  | HTTP_status { headers; _ } -> headers
  | _ -> []

let pp fmt t =
  let redacted_uri = Redaction.uri t.context.uri in
  Format.fprintf fmt
    "eta-http error=%s method=%s uri=%s protocol=%s layer=%s retryability=%s \
     error_class=%s"
    (kind_name t.kind) t.context.method_ redacted_uri
