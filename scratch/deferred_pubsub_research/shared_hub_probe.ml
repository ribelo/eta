...
type ('a, 'err) publisher = {
  value : 'a;
  resolver : ('a, 'err) publish_out Eio.Promise.u;
  mutable active : bool;
}
...
let create ~overflow () =
  ...
  | Drop_new { capacity } | Backpressure { capacity } ->
      if capacity <= 0 then
        invalid_arg
          "Shared_hub_probe.create: bounded capacity must be > 0");
  {
...
let decrement_entry (entry : ('a, 'err) entry) =
  if entry.remaining <= 0 then
    invalid_arg "Shared_hub_probe: negative remaining subscriber count";
  ...
  if not sub.active then `Closed
    ...
    | None ->
        if sub.cursor < t.next_seq then
          invalid_arg "Shared_hub_probe: subscriber cursor points behind buffer";
        (match t.closed with
        | None -> `Empty
