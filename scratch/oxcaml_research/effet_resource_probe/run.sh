...
probe_dir="scratch/oxcaml_research/effet_resource_probe"
...
run_dune_fixture() {
  ...
  set +e
  EFFET_OXCAML_RESEARCH=true dune exec "$probe_dir/$name.exe" >"$log" 2>&1
  local status=$?
  set -e
...
run_dune_fixture effet_resource_portable_probe pass
run_compile_fixture effect_map_payload_negative fail
run_compile_fixture open_variant_error_negative fail
