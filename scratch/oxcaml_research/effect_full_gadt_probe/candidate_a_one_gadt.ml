(* Candidate A: one full Effect.t-shaped GADT whose parameters are all
   constrained for domain-portable execution.

   This intentionally mirrors packages/effet/effect.ml's constructors, plus
   @@ portable on callback-bearing constructors. *)
...
type ('env : value mod portable contended, 'err : immutable_data, 'a : immutable_data) t =
  ...
  | For_each_par :
      'x list * ('x -> ('env, 'err, 'a) t) @@ portable
      -> ('env, 'err, 'a list) t
  | For_each_par_bounded :
      int * 'x list * ('x -> ('env, 'err, 'a) t) @@ portable
      -> ('env, 'err, 'a list) t
  ...
  | Retry :
      ('env, 'err, 'a) t * Schedule.t * ('err -> bool) @@ portable
      -> ('env, 'err, 'a) t
  | Acquire_release :
      ('env, 'err, 'a) t * ('a -> ('env, 'err, unit) t) @@ portable
      -> ('env, 'err, 'a) t
  ...
  | Supervisor_scoped :
      ...
      -> ('env, 'err, 'a) t
  | Render_error :
      ('err -> string) @@ portable * ('env, 'err, 'a) t
      -> ('env, 'err, 'a) t
  | Named :
...
and ('s, 'env : value mod portable contended, 'err : immutable_data, 'a : immutable_data)
    supervisor_scope =
  ...
  | Supervisor_bind :
      ...
      * ('b -> ('s, 'env, 'err, 'a) supervisor_scope) @@ portable
...
and ('env : value mod portable contended, 'err : immutable_data, 'a : immutable_data)
    supervisor_body = {
  run :
    's.
