...
| 2 | Exercise Eta primitives in real use. | `eta_otel.ml` uses `Stream.merge`, `Stream.flat_map_par`, `Mailbox.to_batch_stream`, `Effect.acquire_release`, `Effect.retry`, `Effect.timeout`, `Effect.race`, `Effect.Private.daemon`, `Resource.t`, `Effect.named`, `Effect.annotate`, and `Capabilities.clock`; `Eta_stream.Drain_counter` covers non-polling flush. `Effect.island` and `Effect.blocking` are documented as not applicable: encoding does not justify CPU offload and transport is Eio TCP. | Proven. |
