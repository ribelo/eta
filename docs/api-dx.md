...
- `Effect.with_background` when a required background child belongs to one
...
`Eta_cache.Refreshable.failures` is not replaced by an `~on_error` callback that
appends to a caller-owned ref. `on_error` is a side-effect hook for immediate
...
`Effect.with_background` when the body does not need a child handle and child
