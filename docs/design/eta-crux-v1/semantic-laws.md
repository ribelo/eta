...
| C-05 | `cutoff x ~equal` suppresses dependent recomputation exactly when `equal old new` is true. It does not suppress root-output delivery after a commit. | `qcheck_cutoff_boundary` |
...
| ID | Law | Gate |
|---|---|---|
| K-01 | `M.compare` defines key identity and `M.bindings` defines output order. | `qcheck_assoc_key_order` |
...
| K-03 | An unequal same-key data change updates the stable data source. It does not rebuild or reset the child. | `qcheck_assoc_data_update` |
...
| K-07 | A changed input map uses at most linear key comparisons and live keyed storage is linear in live keys. | existing `Eta_signal_map` keyed complexity gate plus `bench_eta_crux_assoc` |
...
| T-04 | Model, graph, scopes, lifecycle edges, output, and effect eligibility commit atomically. Failure restores the complete previous snapshot. | `race_commit_atomicity` |
...
| S-01 | An equal source spec preserves the producer and updates committed mappers. An unequal spec creates a fresh incarnation. | `qcheck_source_spec_identity` |
