...
Does Eta Crux need a graph-owned protocol that prevents stale asynchronous Poll
results from changing the current model?
...
capability. If adopted, specify the API shape, run identity, ordering,
...
Adopt a graph-owned `Poll` computation. The result with the greatest committed
Poll-run order is current.

`Poll` starts runs on activation, significant input changes, or manual refresh.
It stores the result with the greatest committed run order. Applications
continue to own other change-triggered effects and coordination policies.
...
Bonsai owns change detection, run order, and stale-result suppression.
...
|---|---|---|
| Graph-owned Poll | Adopt | Start effectful runs and prevent an older committed completion from replacing a newer one. |
| Application sequence tokens | Retain | Express domain-specific ordering outside `Poll`. |
| Cancellation on replacement | Reject | A newer run does not cancel an older run in the same active Poll incarnation. |
...
propagation. It does not control run order.

### Automatic polling

`effect_on_change` starts one run when its Poll incarnation activates. It
...
the candidate input. A suppressed candidate starts no run.

One advancement starts at most one automatic run for one Poll node. Multiple
changes during stabilization coalesce into the latest committed input.

A provider change alone starts no run. A triggering commit captures the latest
provider. Eta Crux invokes that provider only when the run starts.
...
run with the latest activation-frame values.

A `Clock_due` advancement can change a Poll input and trigger one run. Moving
...
Each invocation of the refresh effect starts one separate Poll run.
...
next run order, and stages one Poll effect. Automatic and manual runs share one
run order.

### Run identity and order

Each active Poll incarnation owns one hidden monotonic run order. The
application and test APIs expose no run identity or sequence token.

Each successful triggering commit assigns the next run order. The order defines
run priority even when the Eta scheduler starts bodies in another
order.

Run order never wraps or repeats within one incarnation. Exhaustion raises
...
`Poll` does not serialize run bodies. A later run does not cancel, interrupt, or
wait for an earlier run in the same active incarnation.

### Completion and result selection

When a Poll body returns, Eta Crux submits one hidden completion ingress item
through ordinary blocking FIFO ingress. Completion admission uses one ingress
slot and gets no priority or reserved capacity.

A run is complete for result selection when its completion ingress item commits. The
effect return alone does not make its result current.

The Poll state keeps the greatest committed run order and its result. A
completion changes the result only when its run order is greater than the
stored order.

For runs A then B, A can publish while B remains incomplete. A later B
...
A newer equal result advances the stored run order. `result_cutoff` can
suppress dependent propagation, but it cannot remove this order fence.

A stale completion does not call `result_cutoff`. Completion ingress-item FIFO
order does not change the run-order comparison.

If root closure wins completion admission, Eta Crux discards the result during
normal shutdown. The Poll body then has no completion ingress item to process.
...
Completion and disposal have two legal winners. A completion ingress item ahead
of the disposal ingress item in FIFO can commit its result first.

If disposal wins, queued or later completion ingress items cannot change Poll
state. They can produce the normal `Stale_endpoint` rejection.
...
identity, run order, input history, and provider history.
...
A Poll run appears in the exact effect inventory of its triggering commit.
It follows the existing started-and-settled or discarded-before-start path.

A hidden Poll completion creates its own successful commit. That commit
...
|---|---|---|
| Each incarnation publishes its selected starting output and has empty run history. | Generated graphs cover static children, `bind` replacement, `Assoc` removal, and same-key re-entry with both starting forms. The boundary is committed root output, provider-call witnesses, and stale driver events from old completions. | `qcheck_poll_starting_incarnation` |
| A successful activation stages one run after delivery. One advancement stages at most one automatic run with the latest committed input. | Generated advancements include endpoint Actions, resets, clock events, multiple candidate changes, and failed stabilization. The boundary is committed output, provider-call witnesses, and post-commit observer events. | `qcheck_poll_activation_and_coalescing` |
...
| Completion ingress items share bounded FIFO ingress with application Actions, and one advancement consumes at most one item. | Generated interleavings include endpoint Actions, refreshes, and two or more completions under full and available capacity. The boundary is admission witnesses, driver frames, and committed outputs. | `qcheck_poll_completion_fifo` |
| A completion changes output only when its order exceeds every prior committed completion order. | Generated incarnations start two to five runs and execute every completion permutation. Each case forces a newer-before-older witness. The boundary is controlled completion order and committed outputs. | `qcheck_poll_committed_run_order` |
...
| Run order is strict and never reused within one incarnation. A new incarnation starts a fresh order. | Generated cases vary run count, completion permutation, disposal, and same-key re-entry. The boundary is result selection, committed outputs, and stale driver events. | `qcheck_poll_run_order` |
| Run-order exhaustion rolls back the complete advancement and records a transition failure with the active trigger. | A white-box generator constructs private Poll models at the final valid orders and the exhausted state. It dispatches endpoint, clock, reset, and refresh triggers through the production Poll transition. The boundary is root failure, prior output, provider-call witnesses, and observer events. This gate adds no public test control. | `qcheck_poll_run_order_overflow` |
...
| The observer inventories each Poll run in its triggering commit and records one lifecycle path. | Generated Poll-only and mixed commits force success, interruption, failure, out-of-order settlement, and discard before start. The boundary is the exact observer trace and controlled provider witnesses. | `qcheck_post_commit_effect_observer_poll_lifecycle` |
...
| A graph without Poll allocates no Poll state, run order, endpoint, hook, observer identity, or observer event. | The disabled-path benchmark covers initial start, equal Actions, changing Actions, resets, and clock events. The boundary is exact allocation counters and the existing latency threshold. | `poll_disabled_allocation` |

Generated failures print graph structure, incarnations, inputs, run orders,
...
Eta Crux owns Poll incarnation, run order, trigger coalescing, provider
sampling, result selection, ingress integration, cancellation initiation, and
...
runs. A graph without Poll keeps the capability path inert.
