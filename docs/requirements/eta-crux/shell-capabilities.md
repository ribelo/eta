...
- When application code represents declarative platform state, eta_crux shall
  expose that state through output fragments. ^shell-4k8t
- When command work needs shell-owned behavior, eta_crux shall send a typed
  outbound capability message through an injected sender. ^shell-9m3x
- When the shell reports completion, failure, or user response for
  shell-owned work, eta_crux shall accept that report as an ordinary
  inbound action. ^shell-2p9n
- When a shell integrates with Eta Crux, eta_crux shall expose no shell operation
  for mutating application state other than inbound action dispatch. ^shell-e1c6
- When application code needs correlation for shell-owned work, eta_crux shall
  require the correlation token to be part of the application-defined outbound
  and inbound payloads. ^shell-n7v3
- When shell-owned work must stop, eta_crux shall express the stop request as an
  outbound capability message. ^shell-c4h8
- While an application declares no shell-owned capabilities, eta_crux shall
  produce no outbound capability messages. ^shell-6h4q
