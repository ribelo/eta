...
- When a transition returns scheduled commands, eta_crux shall stage those
  commands only after committing the returned model. ^tick-7v5d
...
- When a tick has pending commands, eta_crux shall spawn those commands after
  fragment observation, and command result actions shall enter the inbound queue
  for a later tick. ^tick-e1v6
- When graph stabilization is in progress and eta_crux receives an action from
  dispatch, a command result, or a subscription item, eta_crux shall enqueue
