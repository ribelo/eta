...
| 5 | Benchmark throughput/latency at-or-better than hand-rolled exporter. | Retained benchmark evidence: `docs/research/evidence/bench-results/eta-otel-encoder-repeat-current.json` and `docs/research/evidence/bench-results/eta-5zo-quick-current.json`. The E2E same-file local collector comparison was transient scratch output and is not retained in Git. | Partially retained. |
