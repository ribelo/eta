...
- `Eta_http_eio.H2.Multiplexer.create_client_reader` owns a reusable
  `Bigstringaf.t` read buffer and pending parser offset/length.
- `Eta_http_eio.H2.Multiplexer.read_client_once` feeds pending bytes to
...
- The focused test builds a real in-process `H2.Client_connection` and
  ...
  `Eta_http_eio.H2.Writer.drain_client`, feeds it to the server, drains the
