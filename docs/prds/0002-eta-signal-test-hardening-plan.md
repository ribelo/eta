...
| T15 | Stream subscription stress | Add stream bridge disposal/subscription stress: consumer blocked waiting during disposal, multiple bridges on one signal, one bridge disposed without affecting another, backpressure plus observer failure releases phase, self-disposal during stream observer update is deterministic. | Medium | Solid/Reactively/Alien subscription cleanup adapted to Eta |
...
| T26 | Stream interruption race | Interrupt a stabilization blocked by `Stream.observe ~capacity:1` backpressure; verify graph lane release, later capacity/dispose cleanup, and later stabilization success. | Medium-high | Eta interruption plus stream bridge |
...
| T28 | Public docs | Update `.mli` docs for public semantic guarantees hardened by tests: explicit stabilization, observer handle role, physical equality, bind invalid scope, observer phase semantics, stream ownership/backpressure, and timer demand ownership. | Medium | Consumer-facing contract |
