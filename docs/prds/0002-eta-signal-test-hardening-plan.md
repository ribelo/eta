...
| T13 | Timer catch-up/coalescing | Large clock jumps for `Time.interval`, `Time.after`, `Time.deadline`, `Time.now`, `Time.step`, and `Time.step_replay`: no past-reschedule loop, no hidden auto-stabilization, and documented exact replay or coalescing behavior is asserted. | High | Incremental clock tests adapted to Eta timers |
...
| T19 | OxCaml extension gates | Add focused compile and runtime tests for the OxCaml modes, stack allocations, unboxed representations, and zero-allocation contracts used by `eta_signal`. | High | Compiler-specific safety |
