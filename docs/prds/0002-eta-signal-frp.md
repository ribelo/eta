...
clock sample for that stabilization. `Time.step` and `Time.step_replay` keep
user code out of stabilization and may remain at the last daemon-published
value until the timer daemon resumes. `Time.step` is the bounded daemon
catch-up default: after the daemon wakes, it calls the user function once with
the elapsed `~missed` cadence count instead of replaying one call per cadence.
`Time.step_replay` is the explicit exact-replay variant and can perform one
call per missed cadence.
...
- `Time.now`, `Time.deadline`, `Time.after`, and `Time.interval` refresh from
  ...
  `Time.step_replay` remain daemon-published until daemon progress.
- `Time.step` is bounded daemon catch-up; exact per-cadence replay is explicit
  as `Time.step_replay`.
...
| Public observer reads are typed effects; no unsafe synchronous read remains | `lib/signal/eta_signal.mli`, `test_observer_read_reports_invalid_state` | Covered |
...
- `Time.step` and `Time.step_replay` callback defects are timer-daemon
