#!/usr/bin/env bash
# Autoresearch benchmark: Eta HTTP/2 server TAIL LATENCY over REAL sockets.
#
# Context: throughput is already ahead of Node h2c (1.10x mean / 1.18x median),
# but Eta's tail latency is the weak spot — H2 p99 is 1.77x Node h2c and 1.46x
# Go TLS. This session optimizes tail latency (p99) while guarding rps + memory
# against regression.
#
# Drives the same standalone Eta h2c server (h2_probe.exe) with oha. We keep the
# identical load shape as the throughput session (1 conn / 16 streams) so the
# numbers stay comparable — only the metric we optimize changes.
#
# Emits `METRIC name=value` lines on stdout.
#
# Primary metric: h2_p99_us_geomean (microseconds, LOWER is better) — geomean of
...
nix develop -c dune build --profile release \
  http-testsuite/test/server_load/h2_probe.exe 2>&1 | tail -20
...
# Load shape: single connection, 16 multiplexed streams (matches the throughput
# session). Tail latency is measured at this concurrency.
N="${ETA_H2_REQUESTS:-40000}"
C=16
P=16
REPS="${ETA_H2_REPS:-3}"
...
run_bench() {
  nix develop -c bash -c '
    ...
    sample() {
      '"$OHA_PIN"' oha --no-tui --output-format json --http-version 2 \
        --redirect 0 --disable-compression -c '"$C"' -p '"$P"' -n '"$N"' "$1" 2>/dev/null \
    ...
    # Warmup (prime oha connection + server steady state).
...
while read -r line; do
  case "$line" in
    ...
    "RESULT "*)
      ...
      echo "METRIC h2_${ep}_p99_us=$p99"
      echo "METRIC h2_${ep}_p50_us=$p50"
      echo "METRIC h2_${ep}_rps=$r"
      [ "$ok" = "1" ] || ALL_OK=0
      ;;
  esac
done <<<"$(echo "$OUT" | grep -E '^RESULT')"

echo "METRIC h2_peak_rss_kb=$RSS_KB"
...
if ok != "1" or any(v <= 0 for v in p99 + p50 + rps):
    print("METRIC h2_p99_us_geomean=0")
    print("METRIC h2_p50_us_geomean=0")
    print("METRIC h2_rps_geomean=0")
    print("METRIC success=0")
else:
    print(f"METRIC h2_p99_us_geomean={gm(p99):.2f}")
    print(f"METRIC h2_p50_us_geomean={gm(p50):.2f}")
    print(f"METRIC h2_rps_geomean={gm(rps):.0f}")
