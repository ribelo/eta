...
let program ~user_query ~user_get ~user_run ~user_fetch ~order_query ~order_get ~order_run ~order_fetch ~cache_query ~cache_get ~cache_run ~cache_fetch =
  ...
  |> Effect.bind (fun acc -> Effect.named "cache_run" (Effect.sync (fun _ -> cache_run acc)))
  |> Effect.bind (fun acc -> Effect.named "cache_fetch" (Effect.sync (fun _ -> cache_fetch acc)))
