...
out_dir="scratch/oxcaml_research/concurrency_model/h3_hardening/t7_timeout_clock/results"
mkdir -p "$out_dir"
: > "$out_dir/compile.out"
...
compile_fixture() {
  ...
  local expect="$2"
  local packages="$3"
  local src="scratch/oxcaml_research/concurrency_model/h3_hardening/t7_timeout_clock/$name.ml"
  local src_base="scratch/oxcaml_research/concurrency_model/h3_hardening/t7_timeout_clock/$name"
...
compile_fixture timeout_clock_positive pass portable,parallel,parallel.scheduler,unix
compile_fixture eio_clock_capture_negative fail eio,eio_main,portable,parallel,parallel.scheduler
compile_fixture random_state_capture_negative fail portable,parallel,parallel.scheduler
