...
out_dir="scratch/oxcaml_research/supervisor_state_probe/results"
mkdir -p "$out_dir"
: > "$out_dir/compile.out"
...
run_fixture() {
  local name="$1"
  local expect="$2"
  local packages="capsule,parallel,parallel.scheduler,portable"
  local src="scratch/oxcaml_research/supervisor_state_probe/$name.ml"
  local src_base="scratch/oxcaml_research/supervisor_state_probe/$name"
...
run_fixture capsule_state_positive pass
run_fixture atomic_state_positive pass
run_fixture atomic_payload_negative fail
