...
let run_one mode f =
  ...
  in
  C.print_summary "release_lock_sleep"
    ([
       ("mode", mode);
       ("verdict", verdict);
...
let () =
  run_one "direct" (fun () -> Stub.release_lock_sleep 0.050);
  run_one "run_in_systhread" (fun () ->
      Eio_unix.run_in_systhread ~label:"release_lock_sleep" (fun () -> Stub.release_lock_sleep 0.050))
