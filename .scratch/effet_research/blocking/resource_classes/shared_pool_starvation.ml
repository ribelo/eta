...
let () =
  ...
  let pool = Pool.create ~name:"shared" (config ~max_threads:4 ~max_queued:256 ()) in
  ignore
    (List.init 100 (fun _ ->
         Eio.Fiber.fork_promise ~sw (fun () ->
             Pool.submit ~label:"fs.scan" pool (fun () -> fs_job ()) ())));
  ...
  C.print_summary "shared_pool_starvation"
    ([
