  ...
  module Extension = struct
    ...
    let keyed_entry_identity :
     ...
     fun owner key ->
      ...
      | Const _ | Var _ | Map _ | Map2 _ | Map3 _ | Map4 _ | Map5 _
      | Map6 _ | Map7 _ | Map8 _ | Map9 _ | All _ | Bind _ ->
          invalid_arg "Finalist_factory.Extension: not a keyed signal"
...
module Raw_benchmark = struct
  ...
  let name t = t.workload_name
  let run_batch t count = t.run count
  let final_read_and_check t = t.check ()

  let stabilize graph =
    if graph.Selected_core.running then
      failwith "selected core reentrant stabilization";
    graph.running <- true;
    match
      Selected_core.begin_pass graph;
      let rec drain () =
        match Selected_core.next_height graph with
        | -1 -> ()
        | height -> (
            match Selected_core.pop graph height with
            | None -> drain ()
            | Some (Selected_core.P node as packed) ->
                (* A retained node may be affected again after its first claim.
                   Releasing its queue stamp here is the opaque edge driver's
                   only mutation of queue ownership. *)
                node.queued_in <- -1;
                ignore (Selected_core.evaluate packed);
                drain ())
      in
      drain ();
      Selected_core.clear_queues graph;
      Selected_core.commit graph;
      Selected_core.clear_admissions graph;
      if graph.phase = Selected_core.Cleanup_pending then
        Selected_core.cleanup graph
    with
    | () -> graph.running <- false
    | exception exn ->
        Selected_core.rollback graph;
        graph.running <- false;
        raise exn

  let scalar ~cutoff depth =
    let module K = Selected_core in
    let graph = K.create () in
    let source = K.var graph 0 in
    let base = K.watch source in
    let base =
      if cutoff then K.map ~cutoff:Int.equal (fun _ -> 0) base
      else base
    in
    let rec chain remaining signal =
      if remaining = 0 then signal
      else chain (remaining - 1) (K.map (( + ) 1) signal)
    in
    let output = chain depth base in
    ignore (K.demand output);
    let expected = ref 0 in
    let run count =
      let first = !expected + 1 in
      for value = first to first + count - 1 do
        K.set graph source value;
        stabilize graph
      done;
      expected := first + count - 1
    in
    {
      workload_name =
      ...
      check =
        (fun () ->
          let observed = K.value output in
          let wanted = if cutoff then depth else !expected + depth in
          if observed <> wanted then
            failwith "raw scalar benchmark produced the wrong value");
    }

  let dynamic () =
    let module S = Selected_core in
  ...
  let input_ops =
    let iter_diff before after emit =
      Int_map.merge
        (fun key left right ->
          (match (left, right) with
          | Some old, None -> emit key (Selected_core.Left old)
          | None, Some value -> emit key (Selected_core.Right value)
          | Some old, Some value when old <> value ->
              emit key (Selected_core.Changed (old, value))
          | _ -> ());
          None)
        before after
      |> ignore
    in
    {
      Selected_core.empty_input = Int_map.empty;
  ...
  let output_ops =
    {
      Selected_core.empty_output = Int_map.empty;
  ...
  let keyed ~kind size =
    ...
    let initial =
      let rec fill key map =
        if key = size then map else fill (key + 1) (Int_map.add key key map)
      in
      fill 0 Int_map.empty
    in
    let source = S.var ~cutoff:(Int_map.equal Int.equal) graph initial in
    let output =
    ...
    let next = ref size in
    let run count =
      for _ = 1 to count do
        current :=
          (match kind with
          | `Data | `Child ->
              let value = Int_map.find 0 !current + 1 in
              Int_map.add 0 value !current
          | `Membership ->
              present := not !present;
              if !present then Int_map.add size size !current
              else Int_map.remove size !current);
        incr next;
    ...
    in
    {
      ...
      check =
        (fun () ->
          ...
          Int_map.iter
            (fun key value ->
              if Int_map.find_opt key observed <> Some (value + 1) then
                failwith "raw keyed benchmark produced the wrong value")
            !current);
    }

  let create = function
    | "changed.depth_1" -> scalar ~cutoff:false 1
    | "changed.depth_10" -> scalar ~cutoff:false 10
    | "changed.depth_100" -> scalar ~cutoff:false 100
    | "cutoff.depth_10" -> scalar ~cutoff:true 10
    | "dynamic.switch" -> dynamic ()
    | "map.data_change.10000" -> keyed ~kind:`Data 10_000
    | "map.data_change.100000" -> keyed ~kind:`Data 100_000
    | "map.membership_change.10000" -> keyed ~kind:`Membership 10_000
    | "map.membership_change.100000" -> keyed ~kind:`Membership 100_000
    | "map.child_change.10000" -> keyed ~kind:`Child 10_000
    | "map.child_change.100000" -> keyed ~kind:`Child 100_000
