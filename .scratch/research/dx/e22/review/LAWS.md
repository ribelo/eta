...
| R79 | `retry` stops on success and retries only while predicate and schedule allow, using runtime delays. | `lib/eta/effect.mli:413-423` | `retry does nothing on initial success`; `retry stops when predicate rejects`; `retry recurs attempts initial plus retries`; `retry schedule uses virtual delays` — `test/core_common/effect_retry_repeat_common_suites.ml:1110-1115,1156-1159` |
| R80 | `retry` passes bare failures and the first typed failure of a catchable composite to its predicate and schedule. | `lib/eta/effect.mli:418-419,425-427` | `retry passes typed failures to schedule`; `retry composite passes first typed failure` — `test/core_common/effect_retry_repeat_common_suites.ml:1116-1119` |
| R81 | `retry` preserves rejection, terminal-exhaustion, and uncatchable composite causes; it does not retry runtime cancellation. | `lib/eta/effect.mli:425-428` | `retry skips composite uncatchable causes`; `retry composite rejection preserves original cause`; `retry composite exhaustion preserves original cause`; `retry does not catch defects`; `retry does not retry cancellation`; `retry does not retry interrupt` — `test/core_common/effect_retry_repeat_common_suites.ml:1120-1133` |
| R82 | `retry_or_else` success, predicate rejection, exhaustion, composite selection, uncatchable boundaries, delay, and fallback replacement follow the documented protocol. | `lib/eta/effect.mli:430-454` | `retry_or_else success`; `retry_or_else eventual success`; `retry_or_else predicate rejection fallback`; `retry_or_else first rejection has no output`; `retry_or_else exhausted fallback`; `retry_or_else fallback failure`; `retry_or_else composite typed failure`; `retry_or_else skips uncatchable causes`; `retry_or_else virtual delays` — `test/core_common/effect_retry_repeat_common_suites.ml:1175-1193` |
...
| CD-E22-006 | `retry_or_else` current-runtime predicate, schedule-policy, and fallback defect/interruption/finalizer failure paths — `effect.mli:436-454`. | Eta scheduling maintainers; extend the `retry_or_else` shared-suite failure matrix; **2026-08-31**. |
