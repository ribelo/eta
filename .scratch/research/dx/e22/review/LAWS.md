...
| R101 | `forever` repeats successes and stops on typed failure, defect, interruption/timeout, or finalizer diagnostic without succeeding. | `lib/eta/effect.mli:508-514` | `forever repeats until timeout`; `forever stops on typed failure`; `forever stops on defect`; `forever stops on finalizer diagnostic` — `test/core_common/effect_retry_repeat_common_suites.ml:1148-1155` |
