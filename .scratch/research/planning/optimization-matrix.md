...
| `eta_schema` | Pure schema decode/encode/transform and JSON representation. | Ready | Benchmarks exist for decode, encode, transform, policy, failures, JSON rendering. | `test/schema_eio`, `eta_schema.*`. | Optimize after stream because AI/provider packages depend on schema shape. |
