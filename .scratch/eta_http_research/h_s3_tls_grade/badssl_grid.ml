...
let () =
      ...
      (fun case ->
        ...
        (match outcome with
         | Handshake_ok { alpn; version } ->
           Printf.printf
             "h_s3_badssl name=%s host=%s expected=%s observed=%s result=%s version=%s alpn=%s\n%!"
             case.name
             case.host
         ...
         | Handshake_error error ->
           Printf.printf
             "h_s3_badssl name=%s host=%s expected=%s observed=%s result=%s detail=%S\n%!"
             case.name
             case.host
             (string_of_expected case.expected)
  ...
  in
  match failures with
  | [] -> Printf.printf "h_s3_badssl_summary verdict=PASS failed=<none>\n%!"
  | failures ->
    Printf.printf
      "h_s3_badssl_summary verdict=FAIL failed=%s\n%!"
