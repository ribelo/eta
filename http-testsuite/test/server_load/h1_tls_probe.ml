(* Standalone Eta HTTPS (H1 over TLS) server probe for autoresearch.
   Generates a local cert, starts the testsuite eta H1/TLS server on a fixed
   port, and runs until killed.
   Usage: h1_tls_probe.exe PORT TEMP_DIR *)
...
let () =
  ...
  match
    Eta_http_testsuite.Eta_server.start ~sw ~env ~port ~temp_dir ~cert_dir
      ~protocol:Eta_http_testsuite.Types.H1
