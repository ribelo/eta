...
type runtime_options = Shared.runtime_options = {
...
let protocol_to_string = Shared.protocol_to_string
let default_max_response_body_bytes = Shared.default_max_response_body_bytes
...
let make ~sw ~net ~clock
  ...
  in
  Shared.make_custom ~protocol:Auto ~request:(auto_request_impl state)
    ~stats:(auto_stats_impl state) ~shutdown:(auto_shutdown_impl state)
