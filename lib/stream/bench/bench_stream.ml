...
let range n = Eta_stream.Stream.range ~start:1 ~stop:n

let range_list n = Eta_stream.Stream.from_iterable (List.init n (fun i -> i + 1))
...
let map_filter_fold_list n =
  range_list n |> Eta_stream.Stream.map (fun x -> x * 2) |> Eta_stream.Stream.filter (fun x -> x mod 3 = 0)
  |> fun s -> run_stream s (Sink.fold ( + ) 0)
