(* Selected issue-11 post-commit edge driver.  This is private probe code. *)
...
module type S = sig
  ...
  val acknowledge : 'a delivery -> bool
  val publish_sealed :
    'a delivery ->
    ('a update -> ('b, 'error) outcome) ->
    ('b, 'error) outcome
...
module Make (Execution : EXECUTION) :
  S with type execution = Execution.t = struct
  ...
  let claim t f =
    match Execution.run t.execution (fun _checkpoint -> Ok (f ())) with
    | Ok value -> value
