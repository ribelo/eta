...
module Select = struct
  ...
  let group_by_many columns query =
    match columns with
    | [] -> invalid_arg "Eta_sql.Select.group_by_many: columns must not be empty"
    | columns ->
  ...
  let limit count query =
    if count < 0 then
      invalid_arg "Eta_sql.Select.limit: count must be non-negative";
...
module Insert = struct
  ...
  let conflict_target columns =
    match columns with
    | [] -> invalid_arg "Eta_sql.Insert.on_conflict: target columns must not be empty"
  ...
  let on_conflict_update columns ~set query =
    match set with
    | [] -> invalid_arg "Eta_sql.Insert.on_conflict_update: set columns must not be empty"
    | set ->
        {
          query with
          conflict =
...
module Eta_schema = struct
  ...
  let create_table ?(if_not_exists = false) (table : _ table) columns =
    if columns = [] then
      invalid_arg "Eta_sql.Eta_schema.create_table: columns must not be empty";
  ...
  let create_index ?(unique = false) ?(if_not_exists = false) ~name
    ...
    if columns = [] then
      invalid_arg "Eta_sql.Eta_schema.create_index: columns must not be empty";
    Create_index
      {
        unique;
        if_not_exists;
