...
let event ?(attrs = []) name =
  make ~leaf_name:"Effect.event" @@ fun frame ->
  let tracing_enabled, _ = Runtime_core.current_tracer frame.runtime in
  (if tracing_enabled then
     match local_get frame RObs.active_span_key with
     | None -> ()
...
let is_tracing_enabled =
  make ~leaf_name:"Effect.is_tracing_enabled" @@ fun frame ->
  let tracing_enabled, _ = Runtime_core.current_tracer frame.runtime in
  ok tracing_enabled

let current_span =
  make ~leaf_name:"Effect.current_span" @@ fun frame ->
  let tracing_enabled, _ = Runtime_core.current_tracer frame.runtime in
...
let current_context =
  make ~leaf_name:"Effect.current_context" @@ fun frame ->
  let tracing_enabled, _ = Runtime_core.current_tracer frame.runtime in
...
let log ?(level = Capabilities.Info) ?(attrs = []) body =
  make ~leaf_name:"Effect.log" @@ fun frame ->
...
let logf ?(level = Capabilities.Info) ?(attrs = []) print =
  make ~leaf_name:"Effect.logf" @@ fun frame ->
  let logging_enabled, logger = Runtime_core.current_logger frame.runtime in
  (if
     logging_enabled
     &&
...
let metric_update ?description ?unit_ ?attrs ~name ~kind value =
  make ~leaf_name:"Effect.metric_update" @@ fun frame ->
...
let metric_updates updates =
  make ~leaf_name:"Effect.metric_updates" @@ fun frame ->
  (if frame.runtime.metrics_enabled then
    let clock = Runtime_core.current_clock frame.runtime in
...
let metric_updates_lazy make_updates =
  make ~leaf_name:"Effect.metric_updates_lazy" @@ fun frame ->
  if frame.runtime.metrics_enabled then
