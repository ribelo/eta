(** Alcotest helpers for Eta Eta_schema tests.

    This package extracts the reusable schema assertions that were previously
    local to [test/schema/run.ml]. It intentionally avoids
...
(** Evaluate the pure Eta effect subset produced by Eta Eta_schema helpers. *)
...
val decode_ok :
  ?name:string -> 'a Eta_schema.Eta_schema.t -> Eta_schema.Json.t -> 'a
(** [decode_ok schema json] decodes or fails with rendered issues. *)

val encode_ok :
  ?name:string -> 'a Eta_schema.Eta_schema.t -> 'a -> Eta_schema.Json.t
...
val check_roundtrip_json :
  ?name:string -> 'a Eta_schema.Eta_schema.t -> Eta_schema.Json.t -> unit
