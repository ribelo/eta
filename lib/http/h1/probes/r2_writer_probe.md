...
- origin-form request target derived from `Http.Core.Url`;
- automatic `Host`;
...
transport path now calls `Http.H1.Write.write_to_flow`, which writes
...
The zero-allocation subject is `Http.H1.Write.write_to_bytes_raw`, a
