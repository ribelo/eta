...
val extract_request : Request.t -> Eta_observability.Trace_context.t option
(** Extract [traceparent], [tracestate], and [baggage] from a request's headers.
    Malformed trace context returns [None], matching
    {!Eta_observability.Trace_context.extract}. *)

val inject_request : Eta_observability.Trace_context.t -> Request.t -> Request.t
