...
let with_span ?(attrs = []) ?(emit_url_full = false) ~protocol request eff =
  ...
  |> Eta.Effect.bind (fun response ->
         ...
         |> Eta_observability.annotate_all (Semconv.response_attrs response))
  |> Eta.Effect.bind_error (fun error ->
         Eta.Effect.fail error
         |> Eta_observability.annotate_all (Semconv.error_attrs error))
  |> Eta_observability.annotate_all request_attrs
  |> Eta_observability.named ~kind:Eta.Capabilities.Client (span_name request)
...
let request ?(enabled = true) ?(emit_url_full = false) ?protocol client request =
  ...
  else Eta_observability.suppress_observability eff
