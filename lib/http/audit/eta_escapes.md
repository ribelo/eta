...
Non-zero sites are valid when classified. Hidden sites are not valid. The
header count is generated by the audit script and is the current raw match
count; the table is the curated classification ledger.
...
```sh
rg -n -t ocaml 'Eio\.Fiber\.fork|Eio\.Switch\.run|Eio\.Promise|Eio\.Mutex|Eio\.Condition|Atomic\.[A-Za-z0-9_]+' lib/http | rg -v 'Atomic\.Portable'
```
...
| `client/client.ml:211-228` | `Eio.Promise` | One-shot bridge from the ocaml-h2 trailer callback into the lazy `Response.trailers` thunk; replace with an Eta-owned one-shot future/promise primitive if Eta grows one. |
