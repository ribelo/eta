...
let test_stats_and_dot_stay_read_only_through_nested_bind_churn () =
  let module Signal = Eta_signal_testable.Make (Observer_error) () in
  with_runtime @@ fun rt ->
  let stats_fields (stats : Signal.stats) =
    [
      ("pure_snapshot_commit_count", stats.Signal.pure_snapshot_commit_count);
      ("callback_delivery_count", stats.Signal.callback_delivery_count);
      ("total_node_count", stats.Signal.total_node_count);
      ("active_observer_count", stats.Signal.active_observer_count);
      ("invalid_observer_count", stats.Signal.invalid_observer_count);
      ("necessary_node_count", stats.Signal.necessary_node_count);
      ("dead_node_count", stats.Signal.dead_node_count);
      ("live_dirty_node_count", stats.Signal.live_dirty_node_count);
      ("recompute_count", stats.Signal.recompute_count);
      ("dynamic_scope_invalidations", stats.Signal.dynamic_scope_invalidations);
      ("nodes_became_necessary", stats.Signal.nodes_became_necessary);
      ("nodes_became_unnecessary", stats.Signal.nodes_became_unnecessary);
      ("stream_bridge_drop_count", stats.Signal.stream_bridge_drop_count);
      ("lane_waiter_count", stats.Signal.lane_waiter_count);
      ("lane_cancelled_waiter_count", stats.Signal.lane_cancelled_waiter_count);
    ]
  in
  let check_stats label expected actual =
    List.iter2
      (fun (expected_name, expected_value) (actual_name, actual_value) ->
        Alcotest.(check string)
          (label ^ " stat name") expected_name actual_name;
        Alcotest.(check int)
          (label ^ " " ^ expected_name) expected_value actual_value)
  ...
  in
  let check_introspection_read_only label =
    let before = run_ok rt (Signal.stats ()) in
    let dot = run_ok rt (Signal.to_dot ~options ()) in
    let after_dot = run_ok rt (Signal.stats ()) in
    check_stats (label ^ " to_dot") before after_dot;
    let after_stats = run_ok rt (Signal.stats ()) in
    check_stats (label ^ " stats") before after_stats;
    dot
  in
  let choose_left = Signal.Var.create true in
  let offset = Signal.Var.create 0 in
  let left = Signal.Var.create 10 in
  let right = Signal.Var.create 100 in
  let captured_left = ref None in
  let selected =
    Signal.bind (Signal.Var.watch choose_left) (fun use_left ->
        Signal.bind (Signal.Var.watch offset) (fun offset ->
            let signal =
              if use_left then
                Signal.Var.watch left |> Signal.map (fun value -> value + offset)
              else
                Signal.Var.watch right
                |> Signal.map (fun value -> value + offset)
            in
            (match (use_left, !captured_left) with
             | true, None -> captured_left := Some signal
             | true, Some _ | false, _ -> ());
            signal))
  in
  let selected_observer =
  ...
  run_ok rt Signal.stabilize;
  let after_initial = run_ok rt (Signal.stats ()) in
  Alcotest.(check int) "two observers are active" 2
    after_initial.Signal.active_observer_count;
  Alcotest.(check int) "selected initial value" 10
    (run_ok rt (Signal.Observer.read selected_observer));
  Alcotest.(check int) "branch initial value" 10
    (run_ok rt (Signal.Observer.read branch_observer));
  let initial_dot = check_introspection_read_only "initial nested bind" in
  Alcotest.(check int) "initial dot shows both observers" 2
    (count_occurrences initial_dot "observer:");
  run_ok rt (Signal.Var.set offset 7);
  run_ok rt (Signal.Var.set choose_left false);
  run_ok rt Signal.stabilize;
  let after_switch = run_ok rt (Signal.stats ()) in
  ...
  Alcotest.(check bool) "nested switch invalidated dynamic scope" true
    (after_switch.Signal.dynamic_scope_invalidations
     > after_initial.Signal.dynamic_scope_invalidations);
  Alcotest.(check bool) "nested switch records dead branch nodes" true
    (after_switch.Signal.dead_node_count > after_initial.Signal.dead_node_count);
  let switch_dot = check_introspection_read_only "after nested bind switch" in
  ...
  Alcotest.(check int) "partial disposal leaves selected observer active" 1
    after_partial_dispose.Signal.active_observer_count;
  Alcotest.(check int) "partial disposal removes invalid observer count" 0
    after_partial_dispose.Signal.invalid_observer_count;
  Alcotest.(check int) "selected observer still reads after partial disposal" 107
    (run_ok rt (Signal.Observer.read selected_observer));
  let partial_dot =
    check_introspection_read_only "after invalid branch disposal"
  in
  ...
  Alcotest.(check int) "final disposal removes active observers" 0
    after_final_dispose.Signal.active_observer_count;
  Alcotest.(check bool) "final disposal releases necessary graph" true
    (after_final_dispose.Signal.necessary_node_count
     < after_partial_dispose.Signal.necessary_node_count);
  let final_dot = check_introspection_read_only "after final disposal" in
