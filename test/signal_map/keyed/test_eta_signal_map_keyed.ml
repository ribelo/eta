...
let expect_defect label f =
  match f () with
  | _ -> Alcotest.failf "%s: expected defect" label
  | exception _ -> ()
...
let test_keyed_mapi_adds_child () =
  ...
  run_ok (S.stabilize ());
  run_ok (S.Var.set input (M.set 2 10 M.empty));
  run_ok (S.stabilize ());
  let actual = run_ok (S.Observer.read observer) |> M.to_list in
...
let test_keyed_mapi_retains_updates_and_removes_child () =
  ...
  let observer = run_ok (S.Observer.observe output ~on_update:(fun _ -> Ok ())) in
  run_ok (S.stabilize ());
  run_ok (S.Var.set input (M.set 1 second M.empty));
  run_ok (S.stabilize ());
  ...
  run_ok (S.Var.set input M.empty);
  run_ok (S.stabilize ());
...
let test_keyed_mapi_child_reads_accepted_data_same_stabilization () =
  ...
  in
  run_ok (S.Var.set local_source 7);
  run_ok (S.Var.set input (M.set 1 second M.empty));
  run_ok (S.stabilize ());
...
let test_keyed_mapi_child_only_change_patches_output () =
  ...
  let observer = run_ok (S.Observer.observe output ~on_update:(fun _ -> Ok ())) in
  run_ok (S.stabilize ());
  run_ok (S.Var.set (Hashtbl.find locals 1) 5);
  run_ok (S.stabilize ());
...
let test_keyed_mapi_builder_defect_rolls_back_and_retries () =
  ...
  let observer = run_ok (S.Observer.observe output ~on_update:(fun _ -> Ok ())) in
  run_ok (S.stabilize ());
  run_ok (S.Var.set input (M.set 1 10 M.empty));
  expect_defect "builder defect" (fun () -> ignore (S.stabilize ()));
...
let test_keyed_mapi_cutoff_defect_rolls_back_and_retries () =
  ...
  let observer = run_ok (S.Observer.observe output ~on_update:(fun _ -> Ok ())) in
  run_ok (S.stabilize ());
  run_ok (S.Var.set input (M.set 1 new_data M.empty));
  expect_defect "cutoff defect" (fun () -> ignore (S.stabilize ()));
