...
module Harness () = struct
  ...
  let observe signal callback =
    run_ok (S.Observer.observe signal ~on_update:callback)
  ...
  let dot options = S.to_dot ~options ()
...
let test_keyed_stats_live_gauges_follow_committed_state () =
  ...
  H.set input (H.M.remove 1 two);
  H.stabilize ();
  let removed = H.stats () in
...
let test_keyed_stats_report_shared_and_independent_diff_work () =
  ...
  let observer = H.observe output (fun _ -> Ok ()) in
  H.stabilize ();
  let before_shared = (H.stats ()).keyed in
  H.set input (H.M.set 1022 (ref 7) initial);
  H.stabilize ();
  let after_shared = (H.stats ()).keyed in
  ...
  in
  let before_independent = (H.stats ()).keyed in
  H.set input independent;
  H.stabilize ();
  let after_independent = (H.stats ()).keyed in
...
let test_keyed_stats_report_affected_child_visits () =
  ...
  let observer = H.observe output (fun _ -> Ok ()) in
  H.stabilize ();
  let before = (H.stats ()).keyed.child_visit_count in
  List.iter (fun key -> H.set (Hashtbl.find locals key) 1) [ 3; 400; 1000 ];
  H.stabilize ();
  let after = (H.stats ()).keyed.child_visit_count in
...
let test_keyed_stats_count_failed_attempt_and_rollback () =
  ...
  let observer = H.observe output (fun _ -> Ok ()) in
  H.stabilize ();
  let baseline = (H.stats ()).keyed in
  fail_builder := true;
  H.set input (H.M.set 1 (ref 1) H.M.empty);
...
let test_keyed_stats_count_each_rolled_back_plan () =
  ...
  let combined =
    H.S.map2
      ...
      first second
  in
  let observer = H.observe combined (fun _ -> Ok ()) in
  H.stabilize ();
  let before = (H.stats ()).keyed in
  fail_downstream := true;
  H.set first_input (H.M.set 1 10 H.M.empty);
  H.set second_input (H.M.set 2 20 H.M.empty);
  (try ignore (H.S.stabilize ()) with _ -> ());
  let after = (H.stats ()).keyed in
...
let test_keyed_stats_commit_transitions_only_after_commit () =
  ...
  let output = H.K.mapi (H.S.Var.watch input) ~f:(fun ~key:_ ~data -> data) in
  let observer =
    H.observe output (fun _ ->
        if !fail_callback then failwith "callback" else Ok ())
  in
  H.stabilize ();
  let before = (H.stats ()).keyed in
  H.set input (H.M.set 1 10 H.M.empty);
  let staged_only = (H.stats ()).keyed in
  ...
  fail_callback := true;
  (try ignore (H.S.stabilize ()) with _ -> ());
  let committed = (H.stats ()).keyed in
  ...
  List.iter
    (fun selected ->
      ...
      Alcotest.(check (option int)) "transaction still commits" (Some 10)
        (H.M.find_opt 1 (H.read observer));
      (match H.S.stats () with
...
let test_keyed_dot_scope_selection_shows_keyed_nodes () =
  ...
  Alcotest.(check bool) "unused absent from necessary" false
    (contains (H.dot necessary) "kind=keyed_mapi");
  Alcotest.(check bool) "unused present in all valid" true
    (contains (H.dot all_valid) "kind=keyed_mapi");
  let observer = H.observe output (fun _ -> Ok ()) in
  H.stabilize ();
  ...
  in
  let dynamic_observer = H.observe dynamic (fun _ -> Ok ()) in
  H.stabilize ();
  H.set active false;
  H.stabilize ();
  let valid_count =
    count_substring (H.dot all_valid) "kind=keyed_mapi"
  in
  let including_invalid_count =
    count_substring (H.dot all_invalid) "kind=keyed_mapi"
  in
  Alcotest.(check bool) "invalid keyed tombstone included" true
    (including_invalid_count > valid_count);
  H.dispose dynamic_observer
...
let test_keyed_dot_invalid_tombstones_are_bounded_and_value_free () =
  ...
  for key = 0 to 1029 do
    H.set input (H.M.set key sentinel H.M.empty);
    H.stabilize ();
    H.set input H.M.empty;
    H.stabilize ()
  done;
  let options =
    H.S.{ default_dot_options with dot_scope = `All_including_invalid }
