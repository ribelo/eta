...
let test_bounded_queue_no_deadlock () =
  ...
  let eff =
    Eta_stream.Stream.merge left right |> Eta_stream.Stream.take 1 |> run_drain
    |> Effect.timeout (Duration.ms 1_000)
  in
  ...
  let clock_stream =
    Eta_stream.Stream.from_effect
      (Effect.named "clock" (Effect.sync (fun () ->
           ...
           1)))
  in
  let db_stream = Eta_stream.Stream.from_effect (Effect.named "db" (Effect.sync (fun () -> db#get))) in
...
let test_grouped_batches () =
  with_runtime @@ fun _env rt ->
  let stream = Eta_stream.Stream.from_iterable [ 1; 2; 3; 4; 5 ] |> Eta_stream.Stream.grouped 2 in
...
let test_from_queue_clean_close_ends_stream () =
  ...
  Alcotest.(check (list int))
    "queued values" [ 1; 2 ]
    (run_ok rt (Eta_stream.Stream.from_queue queue |> run_collect))
