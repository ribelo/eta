...
let check_stream_stats (stats : Eta_http_h2.Stream_state.stats) =
...
let () =
  Crowbar.add_test ~name:"h2 security arbitrary chunks do not escape"
    [ chunk_gen ] (fun chunk ->
      ...
      ignore
        (Eta_http_h2.Security.observe_result security bytes ~off:0 ~len
           ~now_ms:0L
          : Eta_http_h2.Security.observation));

  Crowbar.add_test ~name:"h2 header validation arbitrary pair does not escape"
    [ header_gen ] (fun header ->
      ignore
        (Eta_http_h2.Security.validate_headers [ header ]
          : Eta_http.Error.kind option));

  Crowbar.add_test ~name:"h2 stream state operation sequence invariants"
    [ stream_ops_gen ] (fun ops ->
      let state = Eta_http_h2.Stream_state.create ~max_concurrent:8 in
      let streams = ref [] in
      List.iter
        (fun op ->
          (match op with
          | Open tag -> (
              match Eta_http_h2.Stream_state.open_stream state ~tag with
          ...
          | Complete index -> (
              match pick_stream streams index with
              | None -> ()
              | Some stream -> Eta_http_h2.Stream_state.mark_complete state stream)
          | Release index -> (
              match pick_stream streams index with
              | None -> ()
              | Some stream ->
                  ignore
                    (Eta_http_h2.Stream_state.release state stream
                      : Eta_http_h2.Stream_state.release))
          | Close -> Eta_http_h2.Stream_state.close state);
          check_stream_stats (Eta_http_h2.Stream_state.stats state))
        ops)
