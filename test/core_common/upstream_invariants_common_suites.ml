module Make (B : Eta_runtime_common_tests.Runtime_backend.S) = struct
  ...
  let test_effect_smol_acquire_use_release_timeout_releases () =
    ...
    in
    let promise = B.fork_run ctx rt body in
    wait_for_sleepers clock 2;
    B.adjust_clock clock (Duration.ms 10);
    check_timeout "acquireUseRelease interrupt" (B.await promise);
  ...
  let test_effect_smol_finally_runs_on_timeout_interrupt () =
    ...
    wait_for_sleepers clock 2;
    B.adjust_clock clock (Duration.ms 10);
    check_timeout "ensuring interrupt" (B.await promise);
  ...
  let test_effect_smol_race_cancels_losers_before_return () =
    ...
    in
    let promise = B.fork_run ctx rt (Effect.race (winner :: losers)) in
    wait_for_sleepers clock 5;
    B.adjust_clock clock (Duration.ms 10);
    check_exit_ok Alcotest.int "winner" 0 (B.await promise);
  ...
  let test_effect_smol_all_fail_fast_releases_losing_scopes () =
    ...
    let promise = B.fork_run ctx rt (Effect.all [ slow 1; boom; slow 2 ]) in
    wait_for_sleepers clock 3;
    B.adjust_clock clock (Duration.ms 10);
    check_boom "all fail-fast" (B.await promise);
    Alcotest.(check (list int))
  ...
  let test_zio_timeout_waits_for_resource_release () =
    ...
    wait_until (fun () -> B.is_resolved promise);
    check_timeout "timeout release" (B.await promise);
