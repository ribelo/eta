module Make (B : Eta_runtime_common_tests.Runtime_backend.S) = struct
  ...
  let test_effect_retry_passes_typed_failures_to_schedule () =
    B.with_runtime @@ fun _ctx rt ->
    let attempts = ref 0 in
    let attempt =
      Effect.sync (fun () -> incr attempts)
    ...
    in
    let schedule = Schedule.recur_until (fun (`Again n) -> n >= 2) in
    (match
       B.run rt
         (Effect.retry ~schedule ~while_:(fun (`Again _) -> true) attempt)
     with
    | Exit.Error (Cause.Fail (`Again 2)) -> ()
    | Exit.Error cause ->
        Alcotest.failf "expected terminal second failure, got %a"
          (Cause.pp pp_hidden) cause
  ...
  let test_effect_repeat_passes_successful_values_to_schedule () =
    B.with_runtime @@ fun _ctx rt ->
    let ticks = ref 0 in
    let body =
      Effect.sync (fun () ->
          ...
          !ticks)
    in
    let schedule = Schedule.recur_until (fun value -> value >= 3) in
    Alcotest.(check int) "terminal schedule output" 3
      (run_ok rt (Effect.repeat ~schedule body));
    Alcotest.(check int) "initial plus two repeats" 3 !ticks
    ...
    [
      ( "Effect retry/repeat",
        [
          ...
          Alcotest.test_case "repeat passes successful values to schedule" `Quick
            test_effect_repeat_passes_successful_values_to_schedule;
