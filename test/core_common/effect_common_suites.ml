module Make (B : Eta_runtime_common_tests.Runtime_backend.S) = struct
  ...
  let test_effect_or_else_does_not_catch_uncatchable_causes () =
    ...
    run "finalizer"
      ...
      (function
        | Cause.Finalizer (Cause.Finalizer.Fail { error = _; rendered }) when String.equal rendered "cleanup" -> ()
        | cause ->
  ...
  let test_effect_filter_or_fail_finalizer_diagnostic () =
    ...
    | Exit.Error
        (Cause.Suppressed
          ...
          }) when String.equal rendered "cleanup" ->
        ()
    | Exit.Error cause ->
        Alcotest.failf "expected finalizer diagnostic, got %a"
  ...
  let test_effect_ignore_errors () =
    ...
    let typed_cause cause =
      Spi.Expert.make ~leaf_name:"test.typed-cause" @@ fun _context ->
      Exit.Error cause
    in
    Alcotest.(check unit)
      ...
      (Exit.to_result
         (Exit.Error
            (Cause.suppressed ~primary:(Cause.Fail "body")
               ~finalizer:(Cause.Finalizer.Fail { error = "release"; rendered = "release" }))))
  ...
  let test_effect_finally_suppresses_cleanup_failure_after_defect () =
    ...
    | Exit.Error
        (Cause.Suppressed
          { primary = Cause.Die _; finalizer = Cause.Finalizer.Fail { error = _; rendered } }) when String.equal rendered "<typed failure>" ->
        ()
    | Exit.Error cause ->
