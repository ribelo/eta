...
let routing () =
  Ai_openrouter.routing ~order:[ "anthropic"; "openai" ]
  ...
  [
    item "encode_responses.10k" (fun () ->
        let structured_output =
          Ai_openrouter.structured_output ~name:"weather_answer"
        ...
        repeat 10_000 (fun () ->
            ignore
              (Ai_openrouter.encode_responses ~structured_output ~routing request)));
    item "request.responses.10k" (fun () ->
        let provider =
          Ai_openrouter.provider
            ~attribution:
              (Ai_openrouter.attribution ~referer:"https://eta.example"
                 ~title:"Eta" ())
            ()
        in
        let routing = routing () in
        repeat 10_000 (fun () ->
            ignore
              (Ai_openrouter.responses_request ~routing ~provider
