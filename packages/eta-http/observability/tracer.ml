...
let with_span ?(attrs = []) ~protocol request eff =
  let request_attrs = attrs @ Semconv.request_attrs ~protocol request in
  let body =
    eff
...
let request_with_retry ?(enabled = true) ?policy ?protocol client request =
  ...
  if not enabled then
    ...
    let request_once request =
      ...
      |> with_span ~attrs:(Semconv.retry_attrs ~attempt:!attempt) ~protocol
           request
    in
    ...
    |> Eta.Effect.catch (fun error ->
           Eta.Effect.fail error |> annotate (Semconv.error_attrs error))
    |> annotate (Semconv.request_attrs ~protocol request)
