(* Port of @effect/opentelemetry/test/Metrics.test.ts.
   ...
   Effet's equivalent: Capabilities.meter trait, Effet.Metric_update AST,
   Meter.in_memory + Effet_otel.meter for live OTLP. The test here uses
   Meter.in_memory and asserts on the aggregated points produced via
   Effet_otel.aggregate_points (the same function the OTLP encoder uses,
...
let test_gauge () =
  ...
  let agg = Effet_otel.aggregate_points aggregated in
  Alcotest.(check int) "two distinct attribute sets" 2 (List.length agg);
  let by_attrs target =
    List.find
      (fun (k, _) ->
        let module Mk = Effet_otel.Metric_key in
...
let test_counter () =
  ...
  in
  let _ = Runtime.run rt prog in
  let agg = Effet_otel.aggregate_points (Meter.dump meter) in
  Alcotest.(check int) "two distinct attribute sets" 2 (List.length agg)
...
let test_counter_monotonic () =
  ...
  | [ (key, (value, _, _)) ] ->
      Alcotest.(check string) "monotonic counter name" "counter-inc"
        key.Effet_otel.Metric_key.name;
      (match key.kind with
  ...
  if not (motel_reachable ()) then
    ...
    let exporter =
      Effet_otel.create ~sw ~net ~clock ~host:"127.0.0.1" ~port:27686
        ~service_name:"effet-otel-test-meter"
        ~on_error:(fun _ -> ())
    ...
    let prog =
      Effect.bind
        (fun () ->
          Effect.bind
            (fun () ->
              Effect.metric_update ~name:"effet.demo.gauge"
                ~kind:Capabilities.Gauge (Capabilities.Float 1.5))
            (Effect.metric_update ~name:"effet.demo.counter" ~description:"demo"
    ...
    Alcotest.(check (slist string String.compare))
      "metric names"
      [ "effet.demo.counter"; "effet.demo.gauge" ]
      names
