(* Port of @effect/opentelemetry/test/Tracer.test.ts.
   ...
   on a feature Effet does not yet have (or has by another mechanism), the
   port is translated into the closest Effet idiom and the divergence is
...
let with_otlp_runtime ~host ~port f =
  ...
  let exporter =
    Effet_otel.create ~sw ~net ~clock ~host ~port
      ~service_name:"effet-otel-test-tracer"
...
(* Mirrors `it.effect("withSpan links", ...)`.
   ...
   activating it. Effet does not, so the equivalent move here is to first
...
(* Mirrors `it.effect("nested withSpan sets correct parent chain", ...)`.
   ...
   Effect-TS asserts `child.parent.value.name === "parent"`. Effet's
...
(* Mirrors `it.effect("supervisor sets context", ...)` and the generator
   ...
   AsyncHooksContextManager) carries the active span. Effet uses
   ...
   named span. We assert that here as the Effet equivalent. *)
(* ------------------------------------------------------------------ *)
...
(* Mirrors `it.effect("currentOtelSpan", ...)`.
   ...
   `Tracer.currentOtelSpan` agree on the span identity. Effet does not
