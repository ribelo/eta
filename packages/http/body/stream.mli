...
val of_reader :
  ?release:(unit -> (unit, Http_error.Error.t) Eta.Effect.t) ->
  (unit -> (read_result, Http_error.Error.t) Eta.Effect.t) ->
  t
...
val read_all : ?max_bytes:int -> t -> (bytes, Http_error.Error.t) Eta.Effect.t
(** Read the full stream into memory, failing with [Body_too_large] if more
    than [max_bytes] would be accumulated. *)
val discard : t -> (unit, Http_error.Error.t) Eta.Effect.t
