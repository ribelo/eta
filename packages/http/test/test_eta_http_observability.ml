...
let observability_client ?(protocol = Http.Client.H1) request =
  Http.Client.make_for_test ~protocol ~request
    ~stats:(fun () ->
      Eta.Effect.pure
        {
...
let test_observability_success_get_semconv () =
  Test.with_traced_test_clock @@ fun _sw _clock rt tracer ->
  let client =
    observability_client (fun _ -> Eta.Effect.pure (retry_response 200))
  in
  let request = Http.Request.make "GET" "https://api.example.test:8443/a?b=c" in
  let response =
    Eta.Runtime.run rt (Http.Observability.Tracer.request client request)
...
let test_observability_can_emit_raw_url_full () =
  ...
  ignore
    (Eta.Runtime.run rt
       (Http.Observability.Tracer.request ~emit_url_full:true client request)
    |> Test.Expect.expect_ok);
  let span = find_span "HTTP GET" tracer in
...
let test_observability_dns_error_semconv () =
  Test.with_traced_test_clock @@ fun _sw _clock rt tracer ->
  let error =
    Http.Error.make ~method_:"GET" ~uri:"https://missing.example.test/"
      (Dns_error { host = "missing.example.test"; message = "no such host" })
  in
  let client = observability_client (fun _ -> Eta.Effect.fail error) in
  let request = Http.Request.make "GET" "https://missing.example.test/" in
  Test.Expect.expect_typed_failure
    (Eta.Runtime.run rt (Http.Observability.Tracer.request client request))
    (fun err ->
      match err.Http.Error.kind with Dns_error _ -> true | _ -> false);
  let span = find_span "HTTP GET" tracer in
...
let test_observability_tls_error_semconv () =
  Test.with_traced_test_clock @@ fun _sw _clock rt tracer ->
  let error =
    Http.Error.make ~method_:"GET" ~uri:"https://expired.example.test/"
      (Tls_handshake_error
         { stage = Tls_handshake; message = "certificate expired" })
  in
  let client = observability_client (fun _ -> Eta.Effect.fail error) in
  let request = Http.Request.make "GET" "https://expired.example.test/" in
  Test.Expect.expect_typed_failure
    (Eta.Runtime.run rt (Http.Observability.Tracer.request client request))
    (fun err ->
      match err.Http.Error.kind with
...
let test_observability_retry_success_spans () =
  ...
  let response =
    Eta.Runtime.run rt
      (Http.Observability.Tracer.request_with_retry client request)
    |> Test.Expect.expect_ok
  in
  Alcotest.(check int) "status" 200 response.status;
...
let test_observability_redirect_semconv () =
  ...
  let attrs =
    Http.Observability.Semconv.redirect_attrs ~location ()
  in
...
let test_observability_redirect_semconv_can_emit_raw () =
  let location = "https://api.example.test/next?token=secret#frag" in
  let attrs =
    Http.Observability.Semconv.redirect_attrs ~emit_location_full:true
      ~location ()
  in
...
let test_observability_h2_protocol_attrs () =
  Test.with_traced_test_clock @@ fun _sw _clock rt tracer ->
  let client =
    observability_client ~protocol:Http.Client.H2 (fun _ ->
        Eta.Effect.pure (retry_response 200))
  in
  let request = Http.Request.make "GET" "https://api.example.test/h2" in
  ignore
    (Eta.Runtime.run rt
       (Http.Observability.Tracer.request ~protocol:Http.Client.H2 client
          request)
    |> Test.Expect.expect_ok);
  let span = find_span "HTTP GET" tracer in
...
let test_observability_recursion_disabled () =
  ...
  let client =
    observability_client (fun _ -> Eta.Effect.pure (retry_response 200))
  in
  let request = Http.Request.make "POST" "https://collector.example.test/v1/traces" in
  ignore
    (Eta.Runtime.run rt
       (Http.Observability.Tracer.request ~enabled:false client request)
    |> Test.Expect.expect_ok);
...
let test_observability_pool_stats_meter () =
  ...
  let client =
    observability_client (fun _ -> Eta.Effect.pure (retry_response 200))
  in
  Eta.Runtime.run rt (Http.Observability.Meter.record_client_stats client)
  |> Test.Expect.expect_ok;
