...
let with_h2_server ?max_concurrent handler client_action =
  ...
  in
  let connection =
    Http.H2.Connection.create ~sw ~flow:(flow :> Http.H2.Connection.flow)
      ?max_concurrent ()
  in
  let rt = Eta.Runtime.create ~sw ~clock () in
  Fun.protect
    ~finally:(fun () -> Http.H2.Connection.shutdown connection)
...
let request_effect ?body connection target =
  ...
  let request = Http.Request.make ?body "GET" uri in
  Http.Client.For_test.request_h2_on_connection connection request
    (Http.Request.url request)
  |> Eta.Effect.bind (fun response ->
         Http.Body.Stream.read_all response.body
         |> Eta.Effect.map (fun body ->
                (response.Http.Response.status, Bytes.to_string body)))
...
let open_h2_request connection tag target =
  ...
  in
  match
    Http.H2.Connection.request connection ~tag request
      ~error_handler:(fun _ _ -> ())
      ~response_handler:(fun _ _ _ -> ())
  with
  | Ok opened -> opened
  | Error (Http.H2.Multiplexer.Admission_rejected { limit }) ->
      Alcotest.failf "request %d unexpectedly rejected at limit %d" tag limit
  | Error Http.H2.Multiplexer.Connection_closed ->
      Alcotest.failf "request %d saw closed connection" tag
  | Error (Http.H2.Multiplexer.Request_failed message) ->
...
let test_h2_connection_admission_error_reports_configured_limit () =
  with_h2_server ~max_concurrent:64
    ...
    (fun _clock rt connection ->
      ...
      in
      ignore (held : Http.H2.Multiplexer.opened_request list);
      let request =
        Http.Request.make "GET" "https://api.example.test/overflow"
      in
      match
        Eta.Runtime.run rt
          (Http.Client.For_test.request_h2_on_connection connection request
      ...
      | Eta.Exit.Error
          (Eta.Cause.Fail
            {
              Http.Error.kind = Stream_admission_rejected { limit };
              _;
            }) ->
  ...
  with_h2_server
    ...
    (fun _clock rt connection ->
      let responses =
        ...
        |> Eta.Effect.all |> Eta.Runtime.run rt |> Test.Expect.expect_ok
      in
      List.iteri
        (fun i (status, body) ->
          ...
          Alcotest.(check string) "body"
            ...
            body)
        responses;
      let stats = Http.H2.Connection.stats connection in
...
let blocking_body ?(release = fun () -> Eta.Effect.unit) () =
  ...
  Http.Body.Stream.of_reader ~release (fun () ->
      if !first then (
        first := false;
        Eta.Effect.pure
...
let timeout_error uri =
  Http.Error.make ~protocol:H2 ~method_:"POST" ~uri
    (Connection_protocol_violation
       { kind = "test_timeout"; message = "h2 request timed out" })

let pp_http_error_detail fmt (error : Http.Error.t) =
  match error.kind with
  | Connection_protocol_violation { kind; message } ->
      Format.fprintf fmt "%a detail=%s:%s" Http.Error.pp error kind message
  | _ -> Http.Error.pp fmt error
...
let test_h2_connection_returns_early_response () =
  with_h2_server
    ...
    (fun _clock rt connection ->
      ...
      in
      let response = Eta.Runtime.run rt effect |> Test.Expect.expect_ok in
...
let raw_headers encoder ?(end_stream = false) ~stream_id headers =
  ...
  Http.H2.Frame.header ~length:(String.length block) ~frame_type:Headers
    ~flags ~stream_id
  ^ block

let raw_data ?(end_stream = false) ~stream_id data =
  let flags = if end_stream then 0x1 else 0 in
  Http.H2.Frame.header ~length:(String.length data) ~frame_type:Data ~flags
    ~stream_id
  ^ data
...
let raw_informational_response_server flow =
  ...
  let response =
    String.concat "" [ Http.H2.Frame.settings; early; final; body ]
  in
...
let test_h2_client_classifies_informational_response () =
  Alcotest.(check bool) "100" true
    (Http.Client.For_test.h2_informational_status 100);
  Alcotest.(check bool) "103" true
    (Http.Client.For_test.h2_informational_status 103);
  Alcotest.(check bool) "101 excluded" false
    (Http.Client.For_test.h2_informational_status 101);
  Alcotest.(check bool) "200 final" false
    (Http.Client.For_test.h2_informational_status 200)
