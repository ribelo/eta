...
let test_h1_writer_get_origin_form () =
  ...
  in
  let request =
    Http.H1.Write.to_string ~method_:"GET" ~url
...
let test_h1_writer_fixed_body () =
  let url = Http.Core.Url.of_string "http://example.test/echo" in
  let request =
    Http.H1.Write.to_string ~method_:"POST" ~url ~headers:[]
...
let test_h1_writer_flow_matches_string_writer () =
  let url = Http.Core.Url.of_string "http://example.test/echo" in
  let body =
    Http.H1.Write.Fixed [ Bytes.of_string "abc"; Bytes.of_string "def" ]
  in
  let expected =
    Http.H1.Write.to_string ~method_:"POST" ~url ~headers:[] ~body
  in
  let buffer = Buffer.create 128 in
  let flow = Eio.Flow.buffer_sink buffer in
  let actual =
    match
      Http.H1.Write.write_to_flow flow ~method_:"POST" ~url ~headers:[]
        ~body
    with
  ...
  | Error error, _ | _, Error error -> Alcotest.fail (Http.Error.to_string error)
...
let test_h1_writer_flow_write_failure_is_typed () =
  ...
  with
  | Error { Http.Error.kind = Connection_closed { during = Http_request }; _ } ->
      ()
  | Error error -> Alcotest.fail (Http.Error.to_string error)
...
let test_h1_writer_bytes_matches_string_writer () =
  ...
  let body = Http.H1.Write.Empty in
  let expected =
    Http.H1.Write.to_string ~method_:"GET" ~url ~headers ~body
  in
  let bytes = Bytes.create 512 in
  let actual =
    Http.H1.Write.write_to_bytes bytes ~pos:0 ~method_:"GET" ~url
      ~headers ~body
  in
  match (expected, actual) with
  | Ok expected, Ok len ->
...
let test_h1_writer_bytes_rejects_small_buffer () =
  ...
  match
    Http.H1.Write.write_to_bytes bytes ~pos:0 ~method_:"GET" ~url
      ~headers:[] ~body:Http.H1.Write.Empty
  with
  | Ok _ -> Alcotest.fail "small writer buffer unexpectedly succeeded"
  | Error { Http.Error.kind = Header_invalid { reason }; _ } ->
...
let expect_h1_header_invalid label = function
  | Error { Http.Error.kind = Header_invalid _; _ } -> ()
  | Error error -> Alcotest.failf "%s unexpected error: %s" label (Http.Error.to_string error)
  | Ok wire ->
      Alcotest.(check bool)
...
let test_h1_writer_rejects_header_injection () =
  let url = Http.Core.Url.of_string "http://example.test/injection" in
  List.iter
    (fun (label, headers) ->
      ...
      let flow = Eio.Flow.buffer_sink buffer in
      (match
         Http.H1.Write.write_to_flow flow ~method_:"GET" ~url ~headers
           ~body:Http.H1.Write.Empty
       with
      | Error { Http.Error.kind = Header_invalid _; _ } ->
