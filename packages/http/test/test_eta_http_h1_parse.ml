  ...
  let raw =
    Bytes.of_string
      "HTTP/1.1 200 OK\r\n\
       ...
       helloextra"
  in
  match Http.H1.Parse.parse raw ~len:(Bytes.length raw) with
  | Error error ->
      Alcotest.fail (Http.H1.Parse.parse_error_to_string error)
  | Ok response ->
      Alcotest.(check string) "version" "http/1.1"
        (Http.Core.Version.to_string response.version);
      Alcotest.(check int) "status" 200 response.status;
      Alcotest.(check string) "reason" "OK"
        (Http.H1.Parse.span_to_string raw response.reason);
      Alcotest.(check (list (pair string string)))
        "headers"
        [ ("Content-Type", "text/plain"); ("Content-Length", "5") ]
        (Http.H1.Parse.headers_to_list raw response.headers);
      Alcotest.(check string) "body" "hello"
        (Bytes.to_string (Http.H1.Parse.body_to_bytes raw response))
...
let test_h1_parser_rejects_bad_content_length () =
  ...
  | Error error ->
      Alcotest.(check string)
        ...
        (Http.H1.Parse.parse_error_to_string error)
