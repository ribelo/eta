...
let test_transport_resolve_stream_success () =
  ...
  let url = Http.Core.Url.of_string "https://example.test/path" in
  let target = Http.Transport.Connect.target_of_url url in
  ...
  let result =
    Http.Transport.Connect.resolve_stream ~net ~method_:"GET" target
    |> Eta.Runtime.run rt
...
let test_transport_resolve_stream_empty_is_typed () =
  ...
  let url = Http.Core.Url.of_string "https://missing.example.test/" in
  let target = Http.Transport.Connect.target_of_url url in
  Test.with_test_clock @@ fun _sw _clock rt ->
  match
    Http.Transport.Connect.resolve_stream ~net ~method_:"GET" target
  ...
  | Eta.Exit.Error
      (Eta.Cause.Fail
        { Http.Error.kind = Dns_error { host; message }; _ }) ->
...
let test_transport_connect_tcp_success () =
  ...
  Http.Transport.Connect.connect_tcp ~sw ~net ~method_:"GET" target
  |> Eta.Runtime.run rt
  |> Test.Expect.expect_ok
  |> ignore
...
let test_transport_connect_tcp_failure_is_typed () =
  ...
  | Eta.Exit.Error
      (Eta.Cause.Fail
        { Http.Error.kind = Connect_error { message }; _ }) ->
...
let counted_tls_flow closed : Http.Transport.Connect.tcp_flow =
  Eio.Resource.T
    ( { closed },
...
let test_transport_connect_tls_closes_flow_on_failure () =
  let url = Http.Core.Url.of_string "https://example.test/path" in
  let target =
    { (Http.Transport.Connect.target_of_url url) with host = "bad host" }
  in
  let closed = ref 0 in
  let flow = counted_tls_flow closed in
  Test.with_test_clock @@ fun _sw _clock rt ->
  match
    Http.Transport.Connect.connect_tls ~method_:"GET" target
      flow
    |> Eta.Runtime.run rt
  with
  ...
  | Eta.Exit.Error
      (Eta.Cause.Fail
        { Http.Error.kind = Tls_handshake_error _; _ }) ->
...
let test_transport_dispatch_unsupported_alpn_closes_flow () =
  let request = Http.Request.make "GET" "https://example.test/path" in
  let closed = ref 0 in
  let flow = counted_tls_flow closed in
  Test.with_test_clock @@ fun _sw _clock rt ->
  match
    Http.Client.For_test.dispatch_alpn
