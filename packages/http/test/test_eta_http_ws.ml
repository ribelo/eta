...
let read_ws_frame ~masked flow =
  ...
  | Error error ->
      Alcotest.failf "WebSocket frame decode failed: %s"
        (Http.Ws.Codec.parse_error_to_string error)
...
let expect_client_frame state opcode payload =
  match Http.Ws.Codec.decode ~masked:true (client_frame_after_upgrade state) with
  | Ok ({ Http.Ws.Codec.opcode = actual_opcode; payload = actual_payload; _ }, _) ->
      Alcotest.(check int)
        "opcode" (Http.Ws.Codec.opcode_to_int opcode)
        (Http.Ws.Codec.opcode_to_int actual_opcode);
...
let test_ws_connect_reads_inbound_text () =
  ...
  let url = Http.Core.Url.of_string "http://example.test/realtime?model=x" in
  Test.with_test_clock @@ fun sw _clock rt ->
  let conn =
    Http.Ws.Client.connect_on_flow ~key ~sw ~flow url
    |> Eta.Runtime.run rt |> Test.Expect.expect_ok
  in
  let messages =
...
let test_ws_send_text_masks_client_frame () =
  ...
  let url = Http.Core.Url.of_string "http://example.test/realtime" in
  Test.with_test_clock @@ fun sw _clock rt ->
  let conn =
    Http.Ws.Client.connect_on_flow ~key ~sw ~flow url
    |> Eta.Runtime.run rt |> Test.Expect.expect_ok
  in
  Http.Ws.Client.send_text conn "hello"
  |> Eta.Runtime.run rt |> Test.Expect.expect_ok;
...
let test_ws_close_1011_fails_inbound_stream () =
  ...
  let conn =
    ...
    |> Eta.Runtime.run rt |> Test.Expect.expect_ok
  in
  match Eta.Runtime.run rt (Stream.run_drain (Http.Ws.Client.incoming conn)) with
...
let test_ws_selected_subprotocol () =
  ...
  Test.with_test_clock @@ fun sw _clock rt ->
  let conn =
    Http.Ws.Client.connect_on_flow ~key ~protocols:[ "realtime"; "json" ]
      ~sw ~flow url
    |> Eta.Runtime.run rt |> Test.Expect.expect_ok
  in
  Alcotest.(check (option string)) "protocol" (Some "realtime")
    (Http.Ws.Client.selected_protocol conn)
...
let test_ws_server_masked_frame_is_protocol_error () =
  let key = "dGhlIHNhbXBsZSBub25jZQ==" in
  let masked =
    Http.Ws.Codec.encode ~mask:(Bytes.of_string "\001\002\003\004")
...
let test_ws_connect_real_tcp_echo () =
  ...
  let conn =
    ...
    |> Eta.Runtime.run rt |> Test.Expect.expect_ok
  in
  Http.Ws.Client.send_text conn "alpha"
  |> Eta.Runtime.run rt |> Test.Expect.expect_ok;
  Http.Ws.Client.send_text conn "beta"
  |> Eta.Runtime.run rt |> Test.Expect.expect_ok;
  let messages =
    Http.Ws.Client.incoming conn
