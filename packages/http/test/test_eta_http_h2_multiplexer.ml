...
let test_h2_multiplexer_reads_server_response () =
  ...
  (match Http.H2.Writer.drain_client ~flow:request_flow client with
  | Yield _ -> ()
  ...
  in
  let reader = Http.H2.Multiplexer.create_client_reader ~buffer_size:128 client in
  let rec loop reads =
...
let test_h2_default_reader_accepts_max_sized_data_frame () =
  ...
  in
  let reader = Http.H2.Multiplexer.create_client_reader client in
  let rec loop reads =
...
let test_h2_request_exception_releases_admission () =
  let mux = Http.H2.Multiplexer.create ~max_concurrent:1 () in
  let request =
    H2.Request.create ~scheme:"https"
      ~headers:(H2.Headers.of_list [ ":authority", "api.example.test" ])
      `GET "/raises"
  ...
  in
  let open_bad tag =
    Http.H2.Multiplexer.For_test.request_with_h2_request raising_request mux
      ~tag request
      ~error_handler:(fun _ _ -> ())
      ~response_handler:(fun _ _ _ -> ())
  in
  let expect_request_failed label = function
    | Error (Http.H2.Multiplexer.Request_failed _) -> ()
    | Error (Http.H2.Multiplexer.Admission_rejected _) ->
  ...
  let stats = Http.H2.Multiplexer.stats mux in
...
let h2_body_pump_effect client server =
  Eta.Effect.sync (fun () ->
      ...
      if client_progress || server_progress then Http.H2.Multiplexer.Read 1
      else Http.H2.Multiplexer.Eof 0)

let h2_body_closed_error =
  Http.Error.make ~protocol:H2 ~method_:"GET" ~uri:"https://api.example.test/"
...
let h2_open_streaming_body mux client server held_writer body_ref =
  ...
  in
  let opened =
    Http.H2.Multiplexer.request mux ~tag:1 request
      ...
      ~response_handler:(fun stream response body ->
        ...
        body_ref :=
          Some
            (Http.H2.Multiplexer.body_stream
...
let test_h2_body_stream_releases_on_eof () =
  ...
  let client = Http.H2.Multiplexer.client_connection mux in
  ...
  let body =
    Http.Body.Stream.read_all (h2_response_body "eof" !body_ref)
    |> Eta.Runtime.run rt
...
let test_h2_body_stream_reads_inline_data_after_header_pump () =
  ...
  Test.with_test_clock @@ fun _sw _clock rt ->
  let body =
    Http.Body.Stream.read_all (h2_response_body "inline" !body_ref)
    |> Eta.Runtime.run rt
    |> Test.Expect.expect_ok
  in
...
let test_h2_multiplexer_delivers_response_trailers () =
  ...
  in
  let opened =
    Http.H2.Multiplexer.request mux ~tag:1
  ...
  Test.with_test_clock @@ fun _sw _clock rt ->
  let body =
    Http.Body.Stream.read_all (h2_response_body "trailers" !body_ref)
    |> Eta.Runtime.run rt
    |> Test.Expect.expect_ok
  in
  ...
  Alcotest.(check (option string)) "grpc status" (Some "0")
    (Http.Core.Header.get "grpc-status" early_trailers);
  Alcotest.(check (option string)) "grpc message" (Some "")
    (Http.Core.Header.get "grpc-message" early_trailers)
...
let test_h2_body_stream_discard_releases_active_stream () =
  ...
  let chunk =
    Http.Body.Stream.read body |> Eta.Runtime.run rt
    |> Test.Expect.expect_ok
  in
  Alcotest.(check (option string)) "first chunk" (Some "prefix")
    (Option.map Bytes.to_string chunk);
  Http.Body.Stream.discard body |> Eta.Runtime.run rt
  |> Test.Expect.expect_ok;
  let stats = Http.H2.Multiplexer.stats mux in
  Alcotest.(check int) "active" 0 stats.active;
  Alcotest.(check int) "live" 0 stats.live;
  ...
  List.iteri
    (fun i result ->
      ...
      Alcotest.(check bool) "release complete" true
        (Http.H2.Multiplexer.release mux
...
let test_h2_multiplexer_server_reset_admission_release () =
  ...
  List.iter
    ...
    results;
  let stats_after_reset = Http.H2.Multiplexer.stats mux in
...
let test_h2_multiplexer_client_cancel_releases_stream () =
        ...
        ~response_handler:(fun stream response response_body ->
          ...
          H2.Body.Reader.schedule_read response_body
            ...
            ~on_read:(fun bs ~off ~len ->
              ...
              first.mux_release <-
                Some (Http.H2.Multiplexer.release mux stream)))
    with
    | Ok opened -> opened
    | Error (Http.H2.Multiplexer.Admission_rejected _) ->
  ...
  Alcotest.(check bool) "released active stream" true
    (first.mux_release = Some Http.H2.Stream_state.Queue_rst);
