...
type h2_mux_result = {
  ...
  mutable mux_stream : Http.H2.Multiplexer.stream option;
  mutable mux_release : Http.H2.Stream_state.release option;
}
...
let h2_mux_create ?max_concurrent ?config result () =
  Http.H2.Multiplexer.create ?max_concurrent ?config
...
let h2_schedule_mux_body mux result stream body =
  let rec loop () =
    H2.Body.Reader.schedule_read body
      ~on_eof:(fun () ->
        Http.H2.Multiplexer.mark_complete mux stream;
        result.mux_eof <- true)
      ~on_read:(fun bs ~off ~len ->
...
let h2_open_mux_request ?(meth = `GET) ?body ?(target = "/") ?(tag = 0) mux
  ...
  in
  match
    Http.H2.Multiplexer.request mux ~tag request
  ...
  | Error Http.H2.Multiplexer.Connection_closed -> Error `Connection_closed
...
let h2_frame_header ~length ~frame_type ~flags ~stream_id =
  Http.H2.Frame.header ~length ~frame_type:(Other frame_type) ~flags
    ~stream_id

let h2_uint32 = Http.H2.Frame.uint32

let h2_settings_frame = Http.H2.Frame.settings

let h2_goaway_no_error = Http.H2.Frame.goaway_no_error

let h2_payload = Http.H2.Frame.payload

let h2_observe_security data =
  let security = Http.H2.Security.create () in
  let bs = Bigstringaf.of_string ~off:0 ~len:(String.length data) data in
  Http.H2.Security.observe security bs ~off:0 ~len:(String.length data)
