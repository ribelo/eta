...
let test_h2_multiplexer_buffer_full_is_security_error () =
  let client =
    H2.Client_connection.create
      ...
      ()
  in
  let reader = Http.H2.Multiplexer.create_client_reader ~buffer_size:8 client in
  let partial_headers =
    String.sub
...
let test_h2_security_settings_churn_reader () =
  let client =
    H2.Client_connection.create
      ...
      ()
  in
  let reader = Http.H2.Multiplexer.create_client_reader ~buffer_size:64 client in
  let source =
...
let test_h2_security_header_normalization_edges () =
  ...
  Alcotest.(check bool) "valid" true
    (Option.is_none
       (Http.H2.Security.validate_headers [ "x-good", "value" ]))
