...
type header = {
  name : Http_core.Span.t;
  value : Http_core.Span.t;
}

type response = {
  version : Http_core.Version.t;
  status : int;
  reason : Http_core.Span.t;
  headers : header list;
  body : Http_core.Span.t;
}
...
let span_to_string buf span =
  Bytes.sub_string buf span.Http_core.Span.off span.len
...
let create_raw_headers capacity =
  if capacity <= 0 then invalid_arg "Http.H1.Parse.create_raw_headers";
  {
...
let[@zero_alloc] parse_raw buf ~len ~max_header_bytes ~headers raw =
  if len > Bytes.length buf then invalid_arg "Http.H1.Parse.parse_raw";
  reset_raw raw;
...
let raw_version raw =
  ...
  | 10 -> Http_core.Version.H1_0
  | 11 -> Http_core.Version.H1_1
  | _ -> Http_core.Version.H1_1
...
let response_of_raw headers raw =
  ...
  in
  {
    ...
    reason =
      Http_core.Span.make ~off:raw.reason_off ~len:raw.reason_len;
    headers = build (raw.header_count - 1) [];
    body = Http_core.Span.make ~off:raw.body_off ~len:raw.body_len;
  }
