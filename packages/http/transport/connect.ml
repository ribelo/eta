...
let target_of_url url =
  let scheme = Http_core.Url.scheme url in
  let port = Http_core.Url.effective_port url in
  {
    url;
    scheme;
    host = Http_core.Url.host url;
    port;
    service = string_of_int port;
  }

let dns_error ~method_ target message =
  Http_error.Error.make ~method_ ~uri:(Http_core.Url.to_string target.url)
...
let tls_error ?(stage = Http_error.Error.Tls_handshake) ~method_ target
    message =
  Http_error.Error.make ~method_ ~uri:(Http_core.Url.to_string target.url)
...
let connect_tls ?alpn_protocols ?ca_file ~method_ target flow =
  ...
  Effect.sync (fun () ->
      try
        ...
        | Ok (`Host host) ->
            let config =
              Http_tls.Config.default_client ?alpn_protocols ?ca_file
                ~peer_name:host ()
            in
            let tls = Http_tls.Eio.client_of_flow config ~host flow in
            let alpn = Http_tls.Eio.alpn_protocol tls in
            Ok (tls, alpn)
        | Ok (`Ip ip) ->
            let config =
              Http_tls.Config.default_client ?alpn_protocols ?ca_file ~ip ()
            in
            let tls = Http_tls.Eio.client_of_flow config flow in
            let alpn = Http_tls.Eio.alpn_protocol tls in
            Ok (tls, alpn)
      with exn -> Error (Printexc.to_string exn))
  |> Effect.bind (function
