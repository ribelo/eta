...
The header count is generated by the audit script and is the current raw
match count. The table below is the curated classification ledger; reconcile
it when the raw count changes before making call-site-specific claims.

Search:

```sh
rg -n -t ocaml 'H2\.|Hpack\.|Eio\.|Cstruct\.|Domain_name\.|Ipaddr\.|Bigstringaf\.|Gz\.|De\.' packages/http | rg -v 'Http\.H2\.'
```

| Site | Dependency | What | Replaceable? | Replacement cost |
| --- | --- | --- | --- | --- |
| `tls/config.mli:18` | `domain-name` | Accept typed hostnames for SNI and local OpenSSL peer verification. | structural | medium; replacing requires another validated hostname representation. |
| `tls/config.mli:19` | `ipaddr` | Accept typed IP literals for local OpenSSL peer verification. | structural | medium; replacing requires another validated IP representation. |
...
| `transport/connect.ml:73` | `ipaddr` | Detect IP-literal peers for local OpenSSL peer verification. | structural | medium; replacing requires another validated IP representation. |
| `transport/connect.ml:76` | `domain-name` | Parse DNS host peers for SNI and local OpenSSL peer verification. | structural | medium; replacing requires another validated hostname representation. |
| `transport/connect.ml:79` | `domain-name` | Validate DNS host peers before SNI and local OpenSSL peer verification. | structural | medium; replacing requires another validated hostname representation. |
