...
module Body = Http_body.Stream
module Body_source = Http_body.Source
...
type t = {
  protocol : protocol;
  request_impl : Request.t -> (Response.t, Http_error.Error.t) Eta.Effect.t;
  stats_impl : unit -> (stats, Http_error.Error.t) Eta.Effect.t;
  shutdown_impl : unit -> (unit, Http_error.Error.t) Eta.Effect.t;
}

let protocol_to_string = function H1 -> "h1" | H2 -> "h2" | Auto -> "auto"
let default_max_response_body_bytes =
  Http_h1.Client.default_max_response_body_bytes
...
let h1_body = function
  | Request.Empty -> Http_h1.Client.Empty
  | Fixed chunks -> Http_h1.Client.Fixed chunks
  | Stream body -> Http_h1.Client.Stream body
  | Rewindable_stream { length; make } ->
      Http_h1.Client.Rewindable_stream { length; make }
...
let h1_request_of_request request =
  ...
  | Ok url ->
      Ok
        {
          Http_h1.Client.method_ = request.Request.method_;
          url;
  ...
  | Error message ->
      Error
        (Http_error.Error.make ~method_:request.method_ ~uri:request.uri
           (Connection_protocol_violation { kind = "url"; message }))

let response_of_h1 (response : Http_h1.Client.response) =
  Response.make ~status:response.Http_h1.Client.status
...
let h2_trailer_result request =
  ...
  let resolve_headers headers =
    match Http_h2.Security.validate_headers headers with
...
let request_h2_on_connection connection request url =
  let mux = Http_h2.Connection.mux connection in
  ...
  in
  unregister_failure :=
    Http_h2.Connection.register_failure_handler connection (fun kind ->
  ...
  let close_no_body stream body =
    ...
    Http_h2.Multiplexer.mark_complete mux stream;
    ignore (Http_h2.Multiplexer.release mux stream);
    unregister ()
  in
  let response_body stream body =
    let body, wake =
      Http_h2.Multiplexer.body_stream_async
  ...
  let open_request h2_request =
    Http_h2.Connection.request connection ~tag:0 h2_request
      ~trailers_handler:(fun headers -> resolve_trailers (H2.Headers.to_list headers))
      ~error_handler:(fun stream error ->
        Http_h2.Multiplexer.mark_remote_reset mux
          (Http_h2.Stream_state.id stream);
        let error =
  ...
  | Ok opened ->
      Body_source.with_owned_stream (Request.body_source request.body) (fun upload ->
          ...
          let response_or_writer =
            ...
            | Fixed chunks ->
                Eta.Effect.sync (fun () ->
                    Http_h2.Connection.fork_daemon connection (fun () ->
...
let make_h1 ~sw ~net
  ...
  if max_response_body_bytes < 0 then
    invalid_arg "Http.Client.make_h1: max_response_body_bytes must be >= 0";
  let pools = Hashtbl.create 8 in
  let pool_values () = Hashtbl.fold (fun _ pool acc -> pool :: acc) pools [] in
  let pool_for request =
    let key = Http_h1.Client.origin_key request.Http_h1.Client.url in
    match Hashtbl.find_opt pools key with
    | Some pool -> Eta.Effect.pure pool
    | None ->
        Http_h1.Client.make_pool ~max_response_body_bytes ?ca_file ~sw ~net
          request.url
  ...
  let request_impl request =
    ...
    | Ok request ->
        ...
        |> Eta.Effect.bind (fun pool ->
               Http_h1.Client.request_with_pool pool request)
  ...
  let stats_impl () =
    Eta.Effect.sync (fun () ->
        ...
        |> List.fold_left
             (fun acc pool ->
               let stats = Http_h1.Client.pool_stats pool in
               {
                 protocol = H1;
  ...
  in
  let shutdown_impl () =
    pool_values () |> List.map Http_h1.Client.shutdown_pool |> Eta.Effect.concat
  in
...
let make ~sw ~net
    ?(max_response_body_bytes = default_max_response_body_bytes) ?ca_file () =
  if max_response_body_bytes < 0 then
    invalid_arg "Http.Client.make: max_response_body_bytes must be >= 0";
  let opened = ref 0 in
  ...
  let h2_connection_for target =
    ...
    | Some connection when not (Http_h2.Connection.is_closed connection) ->
        Some connection
    | Some _ ->
  ...
  let h1_on_flow flow request =
    ...
    | Ok h1_request ->
        Http_h1.Client.request_on_flow ~release:(fun () -> close_counted flow)
  ...
  let h2_on_tls target tls request url =
    let key = h2_key target in
    let connection =
      Http_h2.Connection.create ~sw ~flow:(tls :> Connect.tcp_flow)
  ...
  let stats_impl () =
    Eta.Effect.sync (fun () ->
        let h2_stats =
          ...
          |> List.fold_left
               (fun acc connection ->
                 let stats = Http_h2.Connection.stats connection in
                 {
                   acc with
  ...
  let shutdown_impl () =
    Eta.Effect.sync (fun () ->
        h2_connections_values () |> List.iter Http_h2.Connection.shutdown;
