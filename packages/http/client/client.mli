...
val stats : t -> (stats, Http_error.Error.t) Eta.Effect.t
val shutdown : t -> (unit, Http_error.Error.t) Eta.Effect.t
val request : t -> Request.t -> (Response.t, Http_error.Error.t) Eta.Effect.t
val request_with_retry :
  ?policy:Retry.t ->
  t ->
  Request.t ->
  (Response.t, Http_error.Error.t) Eta.Effect.t
...
val make_for_test :
  protocol:protocol ->
  request:(Request.t -> (Response.t, Http_error.Error.t) Eta.Effect.t) ->
  stats:(unit -> (stats, Http_error.Error.t) Eta.Effect.t) ->
  shutdown:(unit -> (unit, Http_error.Error.t) Eta.Effect.t) ->
  t

module For_test : sig
  val dispatch_alpn :
    close:(unit -> (unit, Http_error.Error.t) Eta.Effect.t) ->
    use_h1:(unit -> ('a, Http_error.Error.t) Eta.Effect.t) ->
    use_h2:(unit -> ('a, Http_error.Error.t) Eta.Effect.t) ->
    Request.t ->
    string option ->
    ('a, Http_error.Error.t) Eta.Effect.t

  val h2_informational_status : int -> bool

  val request_h2_on_connection :
    Http_h2.Connection.t ->
    Request.t ->
    Http_core.Url.t ->
    (Response.t, Http_error.Error.t) Eta.Effect.t
end
