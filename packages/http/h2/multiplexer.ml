...
type read_result =
  ...
  | Eof of int
  | Close
  | Security_error of Http_error.Error.kind
...
let create_client_reader ?(buffer_size = 64 * 1024) ?security_config client =
  if buffer_size <= 0 then
    invalid_arg "Http.H2.Multiplexer.create_client_reader: buffer_size must be > 0";
...
let buffer_exhausted reader =
  Security_error
    (Http_error.Error.Connection_protocol_violation
       {
...
let body_stream ?(poll_error = fun () -> None) ?(on_eof = fun () -> ())
  ...
  let emit_event = function
    | Body_chunk chunk -> Eta.Effect.pure (Http_body.Stream.Chunk chunk)
    | Body_eof -> Eta.Effect.pure Http_body.Stream.End
  in
  let closed_or_error () =
    match poll_error () with
    | Some error -> Eta.Effect.fail error
    | None when !eof -> Eta.Effect.pure Http_body.Stream.End
    | None when H2.Body.Reader.is_closed body ->
        finish_eof ();
        Eta.Effect.pure Http_body.Stream.End
  ...
  in
  schedule_read ();
  Http_body.Stream.of_reader ~release:release_body read_next
...
let body_stream_async ?(poll_error = fun () -> None) ?(on_eof = fun () -> ())
  ...
  in
  schedule_read ();
  (Http_body.Stream.of_reader ~release:release_body read_next, notify)
