...
val body_stream :
  ?poll_error:(unit -> Http_error.Error.t option) ->
  ?on_eof:(unit -> unit) ->
  ?on_release:
    (Stream_state.release -> (unit, Http_error.Error.t) Eta.Effect.t) ->
  closed_error:Http_error.Error.t ->
  pump:(unit -> (read_result, Http_error.Error.t) Eta.Effect.t) ->
  t ->
  stream ->
  H2.Body.Reader.t ->
  Http_body.Stream.t
