...
let frame_type_code = function
  ...
  | Other code ->
      if code < 0 || code > 0xff then
        invalid_arg "Http.H2.Frame.frame_type_code: code outside uint8";
      code
...
let validate_range label ~max value =
  if value < 0 || value > max then
    invalid_arg
      (Printf.sprintf "Http.H2.Frame.%s outside 0..%d" label max)
...
let uint32 n =
  if n < 0 then invalid_arg "Http.H2.Frame.uint32: negative value";
...
let payload len =
  if len < 0 then invalid_arg "Http.H2.Frame.payload: negative length";
  String.make len '\000'
