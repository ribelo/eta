...
let otlp_headers config =
  List.fold_left
    (fun headers (name, value) ->
      Http.Core.Header.unsafe_add name value
        (Http.Core.Header.remove name headers))
...
let otlp_request config ~path ~body =
  Http.Request.make ~headers:(otlp_headers config)
...
let post_effect t config ~path ~body =
  ...
  |> Eta.Effect.bind (fun response ->
         Http.Body.Stream.read_all response.Http.Response.body
         |> Eta.Effect.map (fun body ->
                (response.Http.Response.status, body)))
  |> Eta.Effect.catch (fun error ->
         Eta.Effect.fail (`Export_error (Http.Error.to_string error)))
...
let shutdown_http_client t =
  ignore
    (Eta.Runtime.run t.flush_rt
       (Http.Client.shutdown t.http_client
...
let create ~sw ~net ~clock ?(host = "127.0.0.1") ?(port = 4318)
  ...
  in
  let http_client = Http.Client.make_h1 ~sw ~net () in
