(* Port of @effect/opentelemetry/test/Metrics.test.ts.
   ...
   Meter.in_memory + Otel.meter for live OTLP. The test here uses
   Meter.in_memory and asserts on the aggregated points produced via
   Otel.aggregate_points (the same function the OTLP encoder uses,
...
let test_gauge () =
  ...
  let agg = Otel.aggregate_points aggregated in
  Alcotest.(check int) "two distinct attribute sets" 2 (List.length agg);
  let by_attrs target =
    List.find
      (fun (k, _) ->
        let module Mk = Otel.Metric_key in
...
let test_counter () =
  ...
  in
  let _ = Runtime.run rt prog in
  let agg = Otel.aggregate_points (Meter.dump meter) in
  Alcotest.(check int) "two distinct attribute sets" 2 (List.length agg)
...
let test_counter_cumulative_keeps_latest_value () =
  ...
  in
  let _ = Runtime.run rt prog in
  match Otel.aggregate_points (Meter.dump meter) with
  | [ (_key, (value, _, _)) ] ->
...
let test_counter_temporality_json () =
  let body =
    Otel.Internal.encode_metrics_request ~resource_attrs:[] ~scope_name:"test"
      [
