...
let test_exception_stacktrace_exported () =
  ...
  let exporter =
    Otel.create ~sw
      ...
      ()
  in
  let rt = Runtime.create ~sw ~clock ~tracer:(Otel.tracer exporter) () in
  let eff =
...
let test_network_partition_reports_error () =
  ...
  let exporter =
    Otel.create ~sw
      ...
      ()
  in
  emit_span (Otel.tracer exporter) "partitioned";
  Otel.flush ~timeout_s:1.0 exporter;
...
let test_malformed_response_reports_error () =
  ...
  let exporter =
    Otel.create ~sw ~net ~clock ~host:"127.0.0.1" ~port
      ...
      ()
  in
  emit_span (Otel.tracer exporter) "malformed";
  Otel.flush ~timeout_s:1.0 exporter;
...
let test_custom_otlp_headers_are_sent () =
  ...
  let exporter =
    Otel.create ~sw ~net ~clock ~host:"127.0.0.1" ~port
      ...
      ()
  in
  emit_span (Otel.tracer exporter) "custom-headers";
  Otel.flush ~timeout_s:1.0 exporter;
  Alcotest.(check (option string))
    "authorization header" (Some "Bearer test-token")
    (Http.Core.Header.get "authorization" !trace_headers);
  Alcotest.(check (option string))
    "custom team header" (Some "test-team")
    (Http.Core.Header.get "x-honeycomb-team" !trace_headers);
  Alcotest.(check (option string))
    "default content-type" (Some "application/json")
    (Http.Core.Header.get "content-type" !trace_headers)
...
let test_encode_failure_drains_in_flight () =
  ...
  meter#record ~name:"bad.float" ~description:"" ~unit_:"1"
    ...
    ~ts_ms:0;
  Otel.flush ~timeout_s:0.2 exporter;
  Alcotest.(check int) "in-flight drained after encode failure" 0
    (Otel.Internal.in_flight exporter)
...
let test_otlp_retry_excludes_408 () =
  ...
  let exporter =
    Otel.create ~sw ~net ~clock ~host:"127.0.0.1" ~port
      ...
      ()
  in
  emit_span (Otel.tracer exporter) "no-408-retry";
  Otel.flush ~timeout_s:1.0 exporter;
  let trace_hits =
...
let test_otlp_retry_includes_429 () =
  ...
  let exporter =
    Otel.create ~sw ~net ~clock ~host:"127.0.0.1" ~port
      ...
      ()
  in
  emit_span (Otel.tracer exporter) "retry-429";
  Otel.flush ~timeout_s:1.0 exporter;
  let trace_hits =
    !paths |> List.filter (String.equal "/v1/traces") |> List.length
  in
...
let test_slow_collector_flush_timeout () =
  ...
  let exporter =
    Otel.create ~sw ~net ~clock ~host:"127.0.0.1" ~port
      ...
      ()
  in
  emit_span (Otel.tracer exporter) "slow";
  let started = Eio.Time.now clock in
  Otel.flush ~timeout_s:0.02 exporter;
...
let test_backpressure_overflow_drops () =
  ...
  Alcotest.(check bool)
    ...
    (Otel.Internal.dropped exporter > 0);
...
let test_shutdown_closes_queues () =
  ...
  Otel.shutdown ~timeout_s:1.0 exporter;
  let sent_before_closed_offer = !sends in
  emit_span tracer "after-shutdown";
  Otel.flush ~timeout_s:0.05 exporter;
  Alcotest.(check int)
...
let test_self_spans_do_not_reenter_export () =
  ...
  let exporter =
    Otel.create ~sw ~net
      ...
      ()
  in
  emit_span (Otel.tracer exporter) "application-span";
  Otel.flush ~timeout_s:1.0 exporter;
  let self_names =
    Otel.Internal.self_spans exporter |> List.map (fun s -> s.Tracer.name)
  in
...
let live_motel_test net clock =
  Eio.Switch.run @@ fun sw ->
  let exporter =
    Otel.create ~sw ~net ~clock ~host:"127.0.0.1" ~port:27686
