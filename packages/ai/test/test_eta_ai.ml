...
let test_api_key_prints_redacted () =
  ...
  let rendered = Format.asprintf "%a" Redacted.pp key in
...
let test_provider_value_carries_endpoint_auth_and_codecs () =
  ...
  in
  let headers = provider.auth_headers (Redacted.make "sk-test") in
  Alcotest.(check string) "provider name" "openai" provider.name;
  Alcotest.(check string)
    "authorization" "Bearer sk-test"
    (Option.get (Http.Core.Header.get "authorization" headers));
...
let test_toolkit_registers_tools_in_order () =
  ...
  in
  let names =
    List.map (fun (tool : Ai.tool) -> tool.name) (toolkit_tools toolkit)
  in
...
let body_of_string ?release value =
  match release with
  | Some release -> Http.Body.Stream.of_bytes ~release (chunk_string value)
  | None -> Http.Body.Stream.of_bytes (chunk_string value)
...
let test_stream_rejects_oversized_complete_record_before_decode () =
  ...
  let body = Http.Body.Stream.of_bytes [ Bytes.of_string payload ] in
...
let chunk_then_eof_body ?release value =
  let reads = ref [ Http.Body.Stream.Chunk (Bytes.of_string value); End ] in
...
let test_stream_decode_error_suppresses_close_failure () =
  with_runtime @@ fun rt ->
  let close_error =
    Http.Error.make ~method_:"GET" ~uri:"https://stream.example"
      (Http.Error.Connection_closed
         { during = Http.Error.Http_response })
  in
  let body =
    chunk_then_eof_body
...
let test_api_key_not_recorded_in_spans_or_logs () =
  ...
  let body =
    ...
    Eta.Effect.log
      ~attrs:[ ("authorization", "Bearer " ^ Redacted.value key) ]
