...
let json_testable =
  Alcotest.testable
    (fun fmt json -> Format.pp_print_string fmt (Schema.Json.to_string json))
    Schema.Json.equal

let issue_testable =
  Alcotest.testable
    (fun fmt issue -> Format.pp_print_string fmt (Schema.render_issue issue))
    ( = )
...
let fail_issues name kind issues =
  Alcotest.failf "%s: %s: %s" name kind (Schema.render_issues issues)
...
let decode_ok ?(name = "decode") schema value =
  Schema.Schema.decode schema value |> run_effect |> expect_ok ~name

let encode_ok ?(name = "encode") schema value =
  Schema.Schema.encode schema value |> run_effect |> expect_ok ~name
