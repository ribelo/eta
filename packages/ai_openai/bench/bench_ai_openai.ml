...
let output () =
  Ai_openai.structured_output ~name:"weather_answer" ~schema_json:weather_schema
    ~strict:true ()
  |> expect_ok
...
let workloads =
  ...
  [
    item "encode_chat.10k" (fun () ->
        ...
        repeat 10_000 (fun () ->
            ignore (Ai_openai.encode_chat ~structured_output request)));
    item "encode_responses.10k" (fun () ->
        let structured_output = output () in
        repeat 10_000 (fun () ->
            ignore (Ai_openai.encode_responses ~structured_output request)));
    item "request.responses.10k" (fun () ->
        repeat 10_000 (fun () ->
            ignore
              (Ai_openai.responses_request
                 ~provider:(Ai_openai.provider ~base_url:"https://api.openai.test" ())
