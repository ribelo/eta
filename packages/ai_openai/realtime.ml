...
let client_secret_request ?base_url ~api_key session =
  ...
  in
  Http.Request.make ~headers:(auth_headers api_key)
...
let read_response_body body =
  ...
  |> E.catch (fun error -> E.fail (A.Http_error error))
...
let create_client_secret ?base_url client ~api_key session =
  ...
  |> E.catch (fun error -> E.fail (A.Http_error error))
  |> E.bind (fun (response : Http.Response.t) ->
         read_response_body response.Http.Response.body
         |> E.bind (fun raw ->
...
let widen_ws_error (error : Http.Ws.Client.ws_error) : realtime_error =
...
let connect ?base_url ?safety_identifier ~sw ~net ~api_key ~model () =
  Http.Ws.Client.connect ~headers:(websocket_headers ?safety_identifier api_key)
...
let send_event t event : (unit, realtime_error) E.t =
  ...
  | Stdlib.Error error -> E.fail error
  | Stdlib.Ok raw ->
      Http.Ws.Client.send_text t.ws raw
      |> E.catch (fun error -> E.fail (widen_ws_error error))

let events t =
  Http.Ws.Client.incoming t.ws
...
let close t = Http.Ws.Client.close t.ws
