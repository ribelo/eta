(* Par — public API on top of the heartbeat scheduler.
...
let current_worker () : S.worker =
  ...
  if w == no_worker then
    invalid_arg "Par: not running inside a pool worker (call Pool.run)";
  w
...
module Pool = struct
  ...
  let create ?(n_workers = default_n_workers ())
             ...
             () =
    if n_workers < 1 then
      invalid_arg "Par.Pool.create: n_workers < 1";
    if heartbeat_interval_ns < 1 then
      invalid_arg "Par.Pool.create: heartbeat_interval_ns < 1";
...
module Iter = struct
  ...
  let of_range ?(chunk = default_chunk) ~start ~stop () : int t =
    if stop < start then invalid_arg "Par.Iter.of_range: stop < start";
    let len = stop - start in
  ...
  let of_array_sub ?(chunk = default_chunk) (arr : 'a array) ~start ~stop : 'a t =
    if start < 0 || stop > Array.length arr || stop < start then
      invalid_arg "Par.Iter.of_array_sub: bad indices";
    let len = stop - start in
