...
let test_pool_run_returns_value () =
  let r =
    Par.Pool.with_pool ~n_workers:4 (fun pool ->
      Par.Pool.run pool (fun () -> 42))
  in
...
let test_join_basic () =
  let r =
    Par.run ~n_workers:4 (fun () ->
      let a, b = Par.join (fun () -> 10) (fun () -> 32) in
      a + b)
  in
...
let rec pfib n =
  if n < 2 then n
    ...
    let a, b = Par.join (fun () -> pfib (n - 1)) (fun () -> pfib (n - 2)) in
    a + b

let test_join_fib () =
  let r = Par.run ~n_workers:4 (fun () -> pfib 25) in
...
let test_par_for () =
  ...
  Par.run ~n_workers:4 (fun () ->
    Par.par_for ~start:0 ~stop:(Array.length arr) (fun i -> arr.(i) <- i * 2));
...
let test_par_iter () =
  ...
  Par.run ~n_workers:4 (fun () ->
    Par.par_iter arr (fun x -> Atomic.add sum x |> ignore));
...
let test_par_map () =
  ...
  let arr = Array.init n (fun i -> i) in
  let doubled =
    Par.run ~n_workers:4 (fun () -> Par.par_map arr (fun x -> x * 2))
  in
...
let test_par_mapi () =
  ...
  let arr = Array.init n (fun i -> i) in
  let summed =
    Par.run ~n_workers:4 (fun () -> Par.par_mapi arr (fun i x -> i + x))
  in
...
let test_par_reduce_sum () =
  ...
  let s =
    Par.run ~n_workers:4 (fun () ->
      Par.par_reduce arr ~init:0 ~map:(fun x -> x) ~combine:( + ))
  in
  let expected = n * (n - 1) / 2 in
...
let test_par_reduce_max () =
  ...
  let m =
    Par.run ~n_workers:4 (fun () ->
      Par.par_reduce arr ~init:min_int
        ~map:(fun x -> x)
        ~combine:max)
  in
...
let test_par_sort_random () =
  ...
  Par.run ~n_workers:4 (fun () -> Par.par_sort arr compare);
...
let test_below_threshold_runs_serial () =
  ...
  let doubled =
    Par.run ~n_workers:2 (fun () -> Par.par_map arr (fun x -> x * 2))
  in
...
let test_par_for_deep_recursion () =
  ...
  Par.run ~n_workers:4 (fun () ->
    Par.par_for ~chunk:1 ~start:0 ~stop:n (fun i -> arr.(i) <- i + 1));
  for i = 0 to n - 1 do
    if arr.(i) <> i + 1 then
...
let test_par_map_deep_recursion () =
  ...
  let out =
    Par.run ~n_workers:4 (fun () ->
      Par.par_map ~chunk:1 arr (fun i -> i * 3 + 1))
  in
  for i = 0 to n - 1 do
    if out.(i) <> i * 3 + 1 then
...
let test_par_for_exception_propagates () =
  ...
  (try
     Par.run ~n_workers:4 (fun () ->
       Par.par_for ~chunk:1 ~start:0 ~stop:1000 (fun i ->
...
let test_par_reduce_exception_propagates () =
  ...
  (try
     let _ : int =
       Par.run ~n_workers:4 (fun () ->
         Par.par_reduce ~chunk:1 arr ~init:0
...
let test_join_left_exception () =
  ...
  (try
     let _ : int * int =
       Par.run ~n_workers:2 (fun () ->
         Par.join (fun () -> raise (Boom 1)) (fun () -> 2))
     in
     ()
...
let test_join_right_exception () =
  ...
  (try
     let _ : int * int =
       Par.run ~n_workers:2 (fun () ->
         Par.join (fun () -> 1) (fun () -> raise (Boom 2)))
     in
     ()
...
let test_par_for_uses_multiple_workers () =
  let n = 200_000 in
  let arr = Array.make n 0 in
  Par.run ~n_workers:4 ~heartbeat_interval_ns:10_000 (fun () ->
    Par.par_for ~chunk:1 ~start:0 ~stop:n (fun i ->
...
let rec deep_join_count s e =
  if s >= e then 0
    ...
    let mid = s + ((e - s) / 2) in
    let l, r =
      Par.join (fun () -> deep_join_count s mid)
...
let test_join_very_long () =
  let n = 1 lsl 16 in
  let r = Par.run ~n_workers:4 (fun () -> deep_join_count 0 n) in
...
let test_iter_of_range_sum () =
  let r =
    Par.run ~n_workers:4 (fun () ->
      Par.Iter.(of_range ~start:0 ~stop:10_000 () |> sum))
  in
...
let test_iter_find_any_present () =
  ...
  let r =
    Par.run ~n_workers:4 (fun () ->
      Par.Iter.(of_array arr |> find_any (fun x -> x = 12_345)))
  in
  match r with
  | Some 12_345 -> ()
...
let test_iter_find_any_absent () =
  ...
  let r =
    Par.run ~n_workers:4 (fun () ->
      Par.Iter.(of_array arr |> find_any (fun x -> x < 0)))
  in
...
let test_iter_any_all () =
  ...
  let any_pos =
    Par.run ~n_workers:4 (fun () ->
      Par.Iter.(of_array arr |> any (fun x -> x > 500)))
  in
  let all_nonneg =
    Par.run ~n_workers:4 (fun () ->
      Par.Iter.(of_array arr |> all (fun x -> x >= 0)))
  in
...
let test_iter_min_max () =
  ...
  let min_v =
    Par.run ~n_workers:4 (fun () ->
      Par.Iter.(of_array arr |> min))
  in
  let max_v =
    Par.run ~n_workers:4 (fun () ->
      Par.Iter.(of_array arr |> max))
  in
