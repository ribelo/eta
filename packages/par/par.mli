(** Par — a parallel runtime for OxCaml.

    Par offers fork-join data parallelism on multiple cores.
...
(** {1 Lazy parallel iterators}
    ...
    {[
      Par.run @@ fun () ->
        arr
        |> Par.Iter.of_array
        |> Par.Iter.map (fun x -> x * x)
        |> Par.Iter.filter (fun x -> x mod 3 = 0)
        |> Par.Iter.reduce ~init:0 ~combine:(+)
    ]}
*)
module Iter : sig
  type 'a t
