...
let par_map n =
  ...
  run_pool (fun pool ->
      Par.Pool.run pool (fun () ->
          Par.par_map ~chunk:1024 input (fun x -> (x * 17) lxor (x lsl 1))
          |> checksum_array))
...
let par_reduce n =
  ...
  run_pool (fun pool ->
      Par.Pool.run pool (fun () ->
          Par.par_reduce ~chunk:1024 input ~init:0 ~map:Fun.id ~combine:( + )))

let par_for n =
  let output = Array.make n 0 in
  run_pool (fun pool ->
      Par.Pool.run pool (fun () ->
          Par.par_for ~chunk:1024 ~start:0 ~stop:n (fun i ->
...
let iter_sum n =
  ...
  run_pool (fun pool ->
      Par.Pool.run pool (fun () ->
          input |> Par.Iter.of_array ~chunk:1024 |> Par.Iter.map (fun x -> x + 1)
          |> Par.Iter.filter (fun x -> x land 1 = 0)
          |> Par.Iter.sum))
