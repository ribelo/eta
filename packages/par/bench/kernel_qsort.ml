(* K2 — Parallel quicksort.
   ...
   Apples-to-apples note: an earlier draft compared [Par.par_sort]
...
let name = "qsort"
let description =
  "Same 3-way quicksort, serial vs Par.par_sort over a random int array"
...
let run_parallel ~quick pool =
  let n = if quick then n_quick else n_default in
  let arr = make_array n 0xC0FFEE in
  Par.Pool.run pool (fun () -> Par.par_sort arr compare);
  assert (is_sorted arr);
  checksum arr
