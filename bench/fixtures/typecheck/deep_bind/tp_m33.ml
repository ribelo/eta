...
let program services =
  ...
  |> Effect.bind (fun acc -> Effect.sync (fun () -> services#cache_set acc) |> Eta_observability.annotate ~key:"module" ~value:"33" |> Eta_observability.named "m33.named")
  ...
  |> Effect.bind (fun acc -> Effect.map_par ~max_concurrent:2 (fun n -> Effect.sync (fun () -> services#metrics_count n)) [ acc; acc + 1; acc + 2 ] |> Effect.map (List.fold_left ( + ) 0))
