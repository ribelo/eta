...
let program services =
  ...
  |> Effect.bind (fun acc -> Effect.sync (fun () -> services#user_write acc) |> Effect.annotate ~key:"module" ~value:"01" |> Effect.named "m01.named")
  ...
  |> Effect.bind (fun acc -> Effect.for_each_par_bounded ~max:2 [ acc; acc + 1; acc + 2 ] (fun n -> Effect.sync (fun () -> services#audit_log n)) |> Effect.map (List.fold_left ( + ) 0))
