...
let program services =
  ...
  |> Effect.bind (fun acc -> Effect.sync (fun () -> services#user_read acc))
  |> Effect.bind (fun acc -> if false then Effect.fail (`Validation "m26") else Effect.pure acc)
  |> Effect.bind (fun acc -> Effect.sync (fun () -> services#user_write acc) |> Eta_observability.annotate ~key:"module" ~value:"26" |> Eta_observability.named "m26.named")
  |> Effect.bind (fun acc -> Effect.par (Effect.sync (fun () -> services#order_read acc)) (Effect.sync (fun () -> services#order_write acc)) |> Effect.map (fun (a, b) -> a + b))
  |> Effect.bind (fun acc -> Effect.all [ Effect.pure acc; Effect.sync (fun () -> services#billing_charge acc); Effect.sync (fun () -> services#billing_refund acc) ] |> Effect.map (List.fold_left ( + ) 0))
  ...
  |> Effect.bind (fun acc -> Effect.map_par ~max_concurrent:2 (fun n -> Effect.sync (fun () -> services#audit_log n)) [ acc; acc + 1; acc + 2 ] |> Effect.map (List.fold_left ( + ) 0))
