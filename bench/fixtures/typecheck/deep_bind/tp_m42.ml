...
let program services =
  ...
  |> Effect.bind (fun acc -> Effect.sync (fun () -> services#shipment_quote acc) |> Effect.annotate ~key:"module" ~value:"42" |> Effect.named "m42.named")
  ...
  |> Effect.bind (fun acc -> Effect.for_each_par_bounded ~max:2 [ acc; acc + 1; acc + 2 ] (fun n -> Effect.sync (fun () -> services#tenant_lookup n)) |> Effect.map (List.fold_left ( + ) 0))
