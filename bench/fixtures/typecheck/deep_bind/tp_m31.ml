...
let program services =
  ...
  |> Effect.bind (fun acc -> Effect.sync (fun () -> services#audit_log acc) |> Effect.annotate ~key:"module" ~value:"31" |> Effect.named "m31.named")
  ...
  |> Effect.bind (fun acc -> Effect.for_each_par_bounded ~max:2 [ acc; acc + 1; acc + 2 ] (fun n -> Effect.sync (fun () -> services#feature_flag n)) |> Effect.map (List.fold_left ( + ) 0))
