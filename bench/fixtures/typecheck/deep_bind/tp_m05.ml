...
let program services =
  ...
  |> Effect.bind (fun acc -> Effect.sync (fun () -> services#billing_charge acc))
  |> Effect.bind (fun acc -> if false then Effect.fail (`Validation "m05") else Effect.pure acc)
  |> Effect.bind (fun acc -> Effect.sync (fun () -> services#billing_refund acc) |> Eta_observability.annotate ~key:"module" ~value:"05" |> Eta_observability.named "m05.named")
  |> Effect.bind (fun acc -> Effect.par (Effect.sync (fun () -> services#audit_log acc)) (Effect.sync (fun () -> services#cache_get acc)) |> Effect.map (fun (a, b) -> a + b))
  |> Effect.bind (fun acc -> Effect.all [ Effect.pure acc; Effect.sync (fun () -> services#cache_set acc); Effect.sync (fun () -> services#search_query acc) ] |> Effect.map (List.fold_left ( + ) 0))
  ...
  |> Effect.bind (fun acc -> Effect.map_par ~max_concurrent:2 (fun n -> Effect.sync (fun () -> services#notify_send n)) [ acc; acc + 1; acc + 2 ] |> Effect.map (List.fold_left ( + ) 0))
