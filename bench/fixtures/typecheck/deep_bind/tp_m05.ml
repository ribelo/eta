...
let program services =
  ...
  |> Effect.bind (fun acc -> Effect.sync (fun () -> services#billing_refund acc) |> Effect.annotate ~key:"module" ~value:"05" |> Effect.named "m05.named")
  ...
  |> Effect.bind (fun acc -> Effect.for_each_par_bounded ~max:2 [ acc; acc + 1; acc + 2 ] (fun n -> Effect.sync (fun () -> services#notify_send n)) |> Effect.map (List.fold_left ( + ) 0))
