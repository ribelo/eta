...
let program services =
  ...
  |> Effect.bind (fun acc -> Effect.sync (fun () -> services#email_send acc) |> Effect.annotate ~key:"module" ~value:"43" |> Effect.named "m43.named")
  ...
  |> Effect.bind (fun acc -> Effect.for_each_par_bounded ~max:2 [ acc; acc + 1; acc + 2 ] (fun n -> Effect.sync (fun () -> services#rate_limit n)) |> Effect.map (List.fold_left ( + ) 0))
