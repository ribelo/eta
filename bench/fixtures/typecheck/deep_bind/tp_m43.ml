...
let program services =
  ...
  |> Effect.bind (fun acc -> Effect.sync (fun () -> services#shipment_quote acc))
  |> Effect.bind (fun acc -> if false then Effect.fail (`Validation "m43") else Effect.pure acc)
  |> Effect.bind (fun acc -> Effect.sync (fun () -> services#email_send acc) |> Eta_observability.annotate ~key:"module" ~value:"43" |> Eta_observability.named "m43.named")
  |> Effect.bind (fun acc -> Effect.par (Effect.sync (fun () -> services#sms_send acc)) (Effect.sync (fun () -> services#report_build acc)) |> Effect.map (fun (a, b) -> a + b))
  |> Effect.bind (fun acc -> Effect.all [ Effect.pure acc; Effect.sync (fun () -> services#policy_eval acc); Effect.sync (fun () -> services#tenant_lookup acc) ] |> Effect.map (List.fold_left ( + ) 0))
  ...
  |> Effect.bind (fun acc -> Effect.map_par ~max_concurrent:2 (fun n -> Effect.sync (fun () -> services#rate_limit n)) [ acc; acc + 1; acc + 2 ] |> Effect.map (List.fold_left ( + ) 0))
