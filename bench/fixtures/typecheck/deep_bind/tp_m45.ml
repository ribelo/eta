...
let program services =
  ...
  |> Effect.bind (fun acc -> Effect.sync (fun () -> services#report_build acc) |> Eta_observability.annotate ~key:"module" ~value:"45" |> Eta_observability.named "m45.named")
  ...
  |> Effect.bind (fun acc -> Effect.map_par ~max_concurrent:2 (fun n -> Effect.sync (fun () -> services#user_read n)) [ acc; acc + 1; acc + 2 ] |> Effect.map (List.fold_left ( + ) 0))
