...
let program services =
  ...
  |> Effect.bind (fun acc -> Effect.sync (fun () -> services#order_write acc))
  |> Effect.bind (fun acc -> if false then Effect.fail (`Validation "m29") else Effect.pure acc)
  |> Effect.bind (fun acc -> Effect.sync (fun () -> services#billing_charge acc) |> Eta_observability.annotate ~key:"module" ~value:"29" |> Eta_observability.named "m29.named")
  |> Effect.bind (fun acc -> Effect.par (Effect.sync (fun () -> services#billing_refund acc)) (Effect.sync (fun () -> services#audit_log acc)) |> Effect.map (fun (a, b) -> a + b))
  |> Effect.bind (fun acc -> Effect.all [ Effect.pure acc; Effect.sync (fun () -> services#cache_get acc); Effect.sync (fun () -> services#cache_set acc) ] |> Effect.map (List.fold_left ( + ) 0))
  ...
  |> Effect.bind (fun acc -> Effect.map_par ~max_concurrent:2 (fun n -> Effect.sync (fun () -> services#search_query n)) [ acc; acc + 1; acc + 2 ] |> Effect.map (List.fold_left ( + ) 0))
