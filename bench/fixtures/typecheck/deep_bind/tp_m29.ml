...
let program services =
  ...
  |> Effect.bind (fun acc -> Effect.sync (fun () -> services#billing_charge acc) |> Effect.annotate ~key:"module" ~value:"29" |> Effect.named "m29.named")
  ...
  |> Effect.bind (fun acc -> Effect.for_each_par_bounded ~max:2 [ acc; acc + 1; acc + 2 ] (fun n -> Effect.sync (fun () -> services#search_query n)) |> Effect.map (List.fold_left ( + ) 0))
