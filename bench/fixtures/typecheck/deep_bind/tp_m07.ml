...
let program services =
  ...
  |> Effect.bind (fun acc -> Effect.sync (fun () -> services#cache_get acc) |> Effect.annotate ~key:"module" ~value:"07" |> Effect.named "m07.named")
  ...
  |> Effect.bind (fun acc -> Effect.for_each_par_bounded ~max:2 [ acc; acc + 1; acc + 2 ] (fun n -> Effect.sync (fun () -> services#config_get n)) |> Effect.map (List.fold_left ( + ) 0))
