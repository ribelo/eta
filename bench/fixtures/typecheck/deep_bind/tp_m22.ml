...
let program services =
  ...
  |> Effect.bind (fun acc -> Effect.sync (fun () -> services#tenant_lookup acc) |> Effect.annotate ~key:"module" ~value:"22" |> Effect.named "m22.named")
  ...
  |> Effect.bind (fun acc -> Effect.for_each_par_bounded ~max:2 [ acc; acc + 1; acc + 2 ] (fun n -> Effect.sync (fun () -> services#order_read n)) |> Effect.map (List.fold_left ( + ) 0))
