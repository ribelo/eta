...
let program services =
  ...
  |> Effect.bind (fun acc -> Effect.sync (fun () -> services#rate_limit acc) |> Eta_observability.annotate ~key:"module" ~value:"23" |> Eta_observability.named "m23.named")
  ...
  |> Effect.bind (fun acc -> Effect.map_par ~max_concurrent:2 (fun n -> Effect.sync (fun () -> services#order_write n)) [ acc; acc + 1; acc + 2 ] |> Effect.map (List.fold_left ( + ) 0))
