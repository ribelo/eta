...
let program services =
  ...
  |> Effect.bind (fun acc -> Effect.sync (fun () -> services#search_query acc) |> Eta_observability.annotate ~key:"module" ~value:"09" |> Eta_observability.named "m09.named")
  ...
  |> Effect.bind (fun acc -> Effect.map_par ~max_concurrent:2 (fun n -> Effect.sync (fun () -> services#auth_check n)) [ acc; acc + 1; acc + 2 ] |> Effect.map (List.fold_left ( + ) 0))
