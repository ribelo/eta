...
| `realuse.ts.scope.acquire_release.64` | `realuse.scope.acquire_release.64` | 64 nested `Effect.acquireRelease` inside one `Effect.scoped` ↔ 64 nested `Effect.acquire_release` inside one `Effect.with_scope`. |
