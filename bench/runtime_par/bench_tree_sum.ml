(* Mirror of chili's overhead.rs — measure the cost of [join] on a
   ...
   - [par]  : same recursion using [Par.join].
...
let rec sum_par t =
  ...
  | Node (v, l, r) ->
    let sl, sr =
      Par.join (fun () -> sum_par l) (fun () -> sum_par r)
    in
    v + sl + sr
...
let bench_one ~layers ~iters ~n_workers =
  ...
  (* Par. *)
  let t_par =
    Par.Pool.with_pool ~n_workers (fun pool ->
      let warm = Par.Pool.run pool (fun () -> sum_par t) in
      assert (warm = expected);
      let times =
        List.init iters (fun _ ->
          let r, dt =
            time_ns (fun () -> Par.Pool.run pool (fun () -> sum_par t))
          in
          assert (r = expected);
          dt)
      in
